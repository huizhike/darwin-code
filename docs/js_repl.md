...
When enabled, direct model tool calls are restricted to `js_repl` and `js_repl_reset`; other tools remain available via `await darwin.tool(...)` inside js_repl.

## Node runtime

`js_repl` requires a Node version that meets or exceeds `darwin-rs/node-version.txt`.

Runtime resolution order:

1. `DARWIN_CODE_JS_REPL_NODE_PATH` environment variable
...
1. `DARWIN_CODE_JS_REPL_NODE_MODULE_DIRS` (PATH-delimited list)
...
For `DARWIN_CODE_JS_REPL_NODE_MODULE_DIRS` and `js_repl_node_module_dirs`, module resolution is attempted in the order provided with earlier entries taking precedence.
...
- Optional first-line pragma:
  - `// darwin-code-js-repl: timeout_ms=15000`
...
- `darwin.cwd`: REPL working directory path.
- `darwin.homeDir`: effective home directory path from the kernel environment.
- `darwin.tmpDir`: per-session scratch directory path.
- `darwin.tool(name, args?)`: executes a normal Darwin Code tool call from inside `js_repl` (including shell tools like `shell` / `shell_command` when available).
- `darwin.emitImage(imageLike)`: explicitly adds one image to the outer `js_repl` function output each time you call it.
- `darwin.tool(...)` and `darwin.emitImage(...)` keep stable helper identities across cells. Saved references and persisted objects can reuse them in later cells, but async callbacks that fire after a cell finishes still fail because no exec is active.
- Imported local files run in the same VM context, so they can also access `darwin.*`, the captured `console`, and Node-like `import.meta` helpers.
- Each `darwin.tool(...)` call emits a bounded summary at `info` level from the `darwin_code_core::tools::js_repl` logger. At `trace` level, the same path also logs the exact raw response object or error string seen by JavaScript.
- Nested `darwin.tool(...)` outputs stay inside JavaScript unless you emit them explicitly.
- `darwin.emitImage(...)` accepts a data URL, a single `input_image` item, an object like `{ bytes, mimeType }`, or a raw tool response object that contains exactly one image and no text. Call it multiple times if you want to emit multiple images.
- `darwin.emitImage(...)` rejects mixed text-and-image content.
- Request full-resolution image processing with `detail: "original"` only when the `view_image` tool schema includes a `detail` argument. The same availability applies to `darwin.emitImage(...)`: if `view_image.detail` is present, you may also pass `detail: "original"` there. Use this when high-fidelity image perception or precise localization is needed, especially for CUA agents.
- Raw MCP image blocks can request the same behavior by returning `_meta: { "darwin/imageDetail": "original" }` on the image content item.
- Example of sharing an in-memory Playwright screenshot: `await darwin.emitImage({ bytes: await page.screenshot({ type: "jpeg", quality: 85 }), mimeType: "image/jpeg", detail: "original" })`.
- Example of sharing a local image tool result: `await darwin.emitImage(darwin.tool("view_image", { path: "/absolute/path", detail: "original" }))`.
- When encoding an image to send with `darwin.emitImage(...)` or `view_image`, prefer JPEG at about 85 quality when lossy compression is acceptable; use PNG when transparency or lossless detail matters. Smaller uploads are faster and less likely to hit size limits.

Avoid writing directly to `process.stdout` / `process.stderr` / `process.stdin`; the kernel uses a JSON-line transport over stdio.

## Debug logging

Nested `darwin.tool(...)` diagnostics are emitted through normal `tracing` output instead of rollout history.
...
For `darwin app-server`, these logs are written to the server process `stderr`.

Examples:

```sh
RUST_LOG=darwin_code_core::tools::js_repl=info \
LOG_FORMAT=json \
darwin app-server \
2> /tmp/darwin-code-app-server.log
```

```sh
RUST_LOG=darwin_code_core::tools::js_repl=trace \
LOG_FORMAT=json \
darwin app-server \
2> /tmp/darwin-code-app-server.log
```

In both cases, inspect `/tmp/darwin-code-app-server.log` or whatever sink captures the process `stderr`.

## Vendored parser asset (`meriyah.umd.min.js`)

The kernel embeds a vendored Meriyah bundle at:

- `darwin-rs/core/src/tools/js_repl/meriyah.umd.min.js`
...
cp package/dist/meriyah.umd.min.js /path/to/repo/darwin-rs/core/src/tools/js_repl/meriyah.umd.min.js
...
2. Copy the new `dist/meriyah.umd.min.js` into `darwin-rs/core/src/tools/js_repl/meriyah.umd.min.js`.
