...
remote_path='~/code/darwin-code-sync'
local_exec_server_port="${DARWIN_CODE_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${DARWIN_CODE_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/darwin-code-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/darwin-code-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/darwin-code-sync"
remote_darwin_code_rs="$remote_repo_root/darwin-rs"

cd "${remote_darwin_code_rs}"
cargo build -p darwin-cli --bin darwin

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/darwin exec-server --listen ws://127.0.0.1:0 \
...
echo "Start darwin via: "
printf '  DARWIN_CODE_EXEC_SERVER_URL=ws://127.0.0.1:%s darwin -C %q\n' \
