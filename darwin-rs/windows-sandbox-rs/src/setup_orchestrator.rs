...
pub const OFFLINE_USERNAME: &str = "DarwinCodeSandboxOffline";
pub const ONLINE_USERNAME: &str = "DarwinCodeSandboxOnline";
...
pub fn sandbox_dir(darwin_code_home: &Path) -> PathBuf {
    darwin_code_home.join(".sandbox")
}

pub fn sandbox_bin_dir(darwin_code_home: &Path) -> PathBuf {
    darwin_code_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(darwin_code_home: &Path) -> PathBuf {
    darwin_code_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(darwin_code_home: &Path) -> PathBuf {
    sandbox_dir(darwin_code_home).join("setup_marker.json")
}

pub fn sandbox_users_path(darwin_code_home: &Path) -> PathBuf {
    sandbox_secrets_dir(darwin_code_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, darwin_code_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        darwin_code_home: request.darwin_code_home.to_path_buf(),
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.darwin_code_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(request.darwin_code_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(darwin_code_home: &Path) -> Vec<PathBuf> {
    ...
    let helper_dir = helper_bin_dir(darwin_code_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(darwin_code_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    if policy.has_full_disk_read_access() {
        gather_legacy_full_read_roots(command_cwd, policy, darwin_code_home)
    } else {
        gather_restricted_read_roots(command_cwd, policy, darwin_code_home)
    }
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "DARWIN_CODE_NETWORK_ALLOW_LOCAL_BINDING";
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let candidate = dir.join("darwin-code-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // `darwin-code-resources/` next to `darwin.exe`, so elevation needs to probe
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("darwin-code-resources")
            .join("darwin-code-windows-sandbox-setup.exe");
        if resource_candidate.exists() {
            return resource_candidate;
        }
    }
    PathBuf::from("darwin-code-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(darwin_code_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(darwin_code_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
                ...
                Some(&sandbox_dir(darwin_code_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(darwin_code_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(request.darwin_code_home);
    ...
    run_setup_exe(&payload, needs_elevation, request.darwin_code_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, request.darwin_code_home);
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.darwin_code_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, darwin_code_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to DARWIN_CODE_HOME or anything under DARWIN_CODE_HOME/.sandbox,
    // DARWIN_CODE_HOME/.sandbox-bin, or DARWIN_CODE_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let darwin_code_home_key = canonical_path_key(darwin_code_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(darwin_code_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(darwin_code_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(darwin_code_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != darwin_code_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "DARWIN_CODE_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let darwin_code_home = tmp.path().join("darwin-code-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &darwin_code_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&darwin_code_home)).expect("canonical helper dir");
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_darwin = extra_write_root.join(".darwin");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_darwin).expect("create extra .darwin");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_darwin).expect("canonical extra .darwin"),
                explicit_deny,
            ]
            .into_iter()
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &darwin_code_home);
