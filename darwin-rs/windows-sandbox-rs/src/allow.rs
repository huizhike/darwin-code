        ...
        let add_writable_root =
            |root: PathBuf,
             ...
             add_deny: &mut dyn FnMut(PathBuf)| {
                ...
                for protected_subdir in [".git", ".darwin", ".agents"] {
...
mod tests {
    ...
    #[test]
    fn denies_darwin_code_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let darwin_code_dir = command_cwd.join(".darwin");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&darwin_code_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&darwin_code_dir).unwrap(),
