...
use anyhow::Context;
use anyhow::Result;
use darwin_code_windows_sandbox::ErrorPayload;
use darwin_code_windows_sandbox::ExitPayload;
use darwin_code_windows_sandbox::FramedMessage;
use darwin_code_windows_sandbox::Message;
use darwin_code_windows_sandbox::OutputPayload;
use darwin_code_windows_sandbox::OutputStream;
use darwin_code_windows_sandbox::PipeSpawnHandles;
use darwin_code_windows_sandbox::SandboxPolicy;
use darwin_code_windows_sandbox::SpawnReady;
use darwin_code_windows_sandbox::SpawnRequest;
use darwin_code_windows_sandbox::StderrMode;
use darwin_code_windows_sandbox::StdinMode;
use darwin_code_windows_sandbox::allow_null_device;
use darwin_code_windows_sandbox::convert_string_sid_to_sid;
use darwin_code_windows_sandbox::create_readonly_token_with_caps_from;
use darwin_code_windows_sandbox::create_workspace_write_token_with_caps_from;
use darwin_code_windows_sandbox::decode_bytes;
use darwin_code_windows_sandbox::encode_bytes;
use darwin_code_windows_sandbox::get_current_token_for_restriction;
use darwin_code_windows_sandbox::hide_current_user_profile_dir;
use darwin_code_windows_sandbox::log_note;
use darwin_code_windows_sandbox::parse_policy;
use darwin_code_windows_sandbox::read_frame;
use darwin_code_windows_sandbox::read_handle_loop;
use darwin_code_windows_sandbox::spawn_process_with_pipes;
use darwin_code_windows_sandbox::to_wide;
use darwin_code_windows_sandbox::write_frame;
use std::ffi::c_void;
use std::fs::File;
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.darwin_code_home.clone();
    hide_current_user_profile_dir(req.darwin_code_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_darwin_code_home={}",
            req.cwd.display(),
            req.command,
            req.real_darwin_code_home.display()
        ),
        Some(&req.darwin_code_home),
    );
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, conpty) = darwin_code_windows_sandbox::spawn_conpty_process_as_user(
            h_token,
            &req.command,
            &effective_cwd,
            &req.env,
