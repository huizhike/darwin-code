...
/// Some Responses-compatible models knowingly require lenient parsing;
/// gpt-4.1 is one such case. While we could try to require everyone to pass in a strictness
...
*** Move to: crates/cli/src/bin/darwin.rs
@@ fn run()
-    let args = Args::parse();
-    dispatch(args)
+    let cli = Cli::parse();
