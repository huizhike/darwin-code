...
/// Special argv[1] flag used when the DarwinCode executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `darwin-code-apply-patch` (to avoid forcing
/// `darwin-code-arg0` to depend on `darwin-code-core`), it remains part of the "darwin core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const DARWIN_CODE_CORE_APPLY_PATCH_ARG1: &str = "--darwin-code-run-as-apply-patch";
