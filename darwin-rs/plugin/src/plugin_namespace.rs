...
pub const PLUGIN_MANIFEST_PATH: &str = ".darwin-code-plugin/plugin.json";
...
/// plugin manifest (same `name` rules as full manifest loading in darwin-code-core).
...
mod tests {
    ...
    fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".darwin-code-plugin")).expect("mkdir manifest");
        fs::write(
