...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-darwin-subagent", "review"),
        response_body,
    )
    .await;
    ...
    let darwin_code_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-darwin-subagent").as_deref(),
        Some("review")
    );
    assert_eq!(
...
async fn responses_stream_includes_subagent_header_on_other() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-darwin-subagent", "my-task"),
        response_body,
    )
    .await;
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/huizhike/darwin-code.git",
    ]);
