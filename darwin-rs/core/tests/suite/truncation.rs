...
async fn mcp_tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_darwin_code().with_config(move |config| {
        ...
        servers.insert(
            ...
            darwin_code_config::types::McpServerConfig {
                transport: darwin_code_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_image_output_preserves_image_and_no_text_summary() -> Result<()> {
    ...
    // 1x1 PNG data URL
    let sample_png = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMB/ee9bQAAAABJRU5ErkJggg==";
    ...
    assert_eq!(
        arr[1],
        json!({"type": "input_image", "image_url": sample_png})
    );
