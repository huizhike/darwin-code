...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.darwin_code);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.darwin_code, "after resume").await?;
    shutdown_thread(&resumed.darwin_code).await?;
...
async fn submit_user_turn(darwin_code: &Arc<DarwinCodeThread>, text: &str) -> Result<()> {
    darwin_code
        .submit(Op::UserInput {
...
async fn submit_compact_turn(darwin_code: &Arc<DarwinCodeThread>) -> Result<()> {
    darwin_code.submit(Op::Compact).await?;
    let warning_event =
        wait_for_event(darwin_code, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(darwin_code: &Arc<DarwinCodeThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        .header("x-darwin_code-window-id")
        .expect("missing x-darwin_code-window-id header");
