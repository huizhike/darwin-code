...
async fn provider_model_header_mismatch_emits_warning_event_and_warning_item() -> Result<()> {
    ...
    let reroute = wait_for_event(&test.darwin_code, |event| {
    ...
    let warning = wait_for_event(&test.darwin_code, |event| {
    ...
    let warning_item = wait_for_event(&test.darwin_code, |event| {
        matches!(
            event,
    ...
    let _ = wait_for_event(&test.darwin_code, |event| {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn provider_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn provider_model_header_casing_only_mismatch_does_not_warn() -> Result<()> {
    skip_if_no_network!(Ok(()));
