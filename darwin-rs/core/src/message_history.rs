...
//!
//! The history is stored at `~/.darwin_code/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.darwin_code`.
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.darwin_code/history.jsonl` and ensure the parent directory exists.
