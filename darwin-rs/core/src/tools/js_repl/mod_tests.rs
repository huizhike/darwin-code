...
async fn can_run_js_repl_runtime_tests() -> bool {
    ...
    // the darwin_code-linux-sandbox arg0 dispatch path, which is exercised in
...
void darwin_code.tool("shell_command", {{ command: `sleep 0.35; printf js_repl_unawaited_done > "${{marker}}"` }});
...
const savedTool = darwin_code.tool;
...
const fs = await import("node:fs/promises");
const path = await import("node:path");
const imagePath = path.join(darwin_code.tmpDir, "js-repl-view-image-explicit.png");
const png = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==",
  "base64"
);
...
await darwin_code.emitImage({ bytes: png, mimeType: "image/png" });
"#;
...
const savedEmitImage = darwin_code.emitImage;
...
async fn js_repl_unawaited_emit_image_errors_fail_cell() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
void darwin_code.emitImage({ bytes: new Uint8Array(), mimeType: "image/png" });
console.log("cell-complete");
"#;
...
async fn js_repl_caught_emit_image_error_does_not_fail_cell() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let code = r#"
try {
  await darwin_code.emitImage({ bytes: new Uint8Array(), mimeType: "image/png" });
} catch (error) {
...
await darwin_code.emitImage({ bytes: png });
"#;
...
async fn js_repl_emit_image_rejects_non_data_url() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
await darwin_code.emitImage("https://example.com/image.png");
"#;
...
async fn js_repl_emit_image_accepts_case_insensitive_data_url() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
await darwin_code.emitImage("DATA:image/png;base64,AAA");
"#;
...
await darwin_code.emitImage({ bytes: png, mimeType: "image/png", detail: "ultra" });
"#;
...
await darwin_code.emitImage({ bytes: png, mimeType: "image/png", detail: null });
"#;
...
async fn js_repl_emit_image_rejects_mixed_content() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
const out = await darwin_code.tool("inline_image", {});
await darwin_code.emitImage(out);
"#;
...
async fn js_repl_dynamic_tool_response_preserves_js_line_separator_text() -> anyhow::Result<()> {
    ...
    for (tool_name, description, expected_text, literal) in [
        ...
        *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
        ...
        let code = format!(
            r#"
const out = await darwin_code.tool("{tool_name}", {{}});
...
async fn js_repl_can_call_hidden_dynamic_tools() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
const out = await darwin_code.tool("hidden_dynamic_tool", { city: "Paris" });
...
async fn js_repl_imported_local_files_can_access_repl_globals() -> anyhow::Result<()> {
    ...
    let cwd_dir = tempdir()?;
    let expected_home_dir = serde_json::to_string("/tmp/darwin_code-home")?;
    write_js_repl_test_module(
        cwd_dir.path(),
        "globals.js",
        &format!(
            "const expectedHomeDir = {expected_home_dir};\nconsole.log(`tmp:${{darwin_code.tmpDir === tmpDir}}`);\nconsole.log(`cwd:${{typeof darwin_code.cwd}}:${{darwin_code.cwd.length > 0}}`);\nconsole.log(`home:${{darwin_code.homeDir === expectedHomeDir}}`);\nconsole.log(`tool:${{typeof darwin_code.tool}}`);\nconsole.log(\"local-file-console-ok\");\n"
        ),
    )?;
    ...
    session
        .set_dependency_env(HashMap::from([(
            "HOME".to_string(),
            "/tmp/darwin_code-home".to_string(),
        )]))
        .await;
    turn.shell_environment_policy
        .r#set
