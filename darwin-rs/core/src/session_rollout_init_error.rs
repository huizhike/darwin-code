...
fn map_rollout_io_error(io_err: &std::io::Error, darwin_code_home: &Path) -> Option<DarwinCodeErr> {
    ...
    let hint = match io_err.kind() {
        ErrorKind::PermissionDenied => format!(
            "DarwinCode cannot access session files at {} (permission denied). If sessions were created using sudo, fix ownership: sudo chown -R $(whoami) {}",
            sessions_dir.display(),
            darwin_code_home.display()
        ),
        ErrorKind::NotFound => format!(
            "Session storage missing at {}. Create the directory or choose a different DarwinCode home.",
            sessions_dir.display()
        ),
        ErrorKind::AlreadyExists => format!(
            "Session storage path {} is blocked by an existing file. Remove or rename it so DarwinCode can create sessions.",
            sessions_dir.display()
        ),
        ...
        ErrorKind::IsADirectory | ErrorKind::NotADirectory => format!(
            "Session storage path {} has an unexpected type. Ensure it is a directory DarwinCode can use for session files.",
            sessions_dir.display()
        ),
        _ => return None,
    };
