...
pub struct Config {
    ...
    ///
    /// - `None`: use default attribution (`DarwinCode <noreply@darwin.local>`)
...
pub(crate) fn ensure_default_byok_provider_config_toml_for_tests(cfg: &mut ConfigToml) {
    ...
    cfg.openai_compatible.insert(
        ...
        darwin_code_config::config_toml::OpenAiCompatibleProviderToml {
            preset: None,
            name: Some("Test OpenAI-compatible gateway".to_string()),
    ...
    if cfg.model_provider.is_none() {
        cfg.model_provider = Some("test-gateway".to_string());
    }
}
...
/// `config.toml` files are not mistaken for DarwinCode home directories.
