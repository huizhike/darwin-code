...
fn external_and_removed_config_surfaces_do_not_block_config_toml_load() {
    ...
    // DarwinCode shares ~/.darwin/config.toml with upstream DarwinCode/OMX installs.
...
#[test]
fn find_darwin_code_home_prefers_darwin_code_home_env() -> std::io::Result<()> {
    let darwin_code_home = TempDir::new()?;
...
fn find_darwin_code_home_detects_source_checkout_from_root() -> std::io::Result<()> {
    let darwin_code_home = TempDir::new()?;
    write_darwin_code_source_home(darwin_code_home.path())?;

    let resolved = find_darwin_code_home_from_env_and_cwd(None, Some(darwin_code_home.path()))?;
...
#[test]
fn find_darwin_code_home_detects_source_checkout_from_darwin_code_rs_child() -> std::io::Result<()>
{
    let source_home = TempDir::new()?;
    write_darwin_code_source_home(source_home.path())?;
    let child = source_home
        .path()
        .join("darwin-rs")
        .join("core")
        .join("src");
...
"#,
    ...
    assert!(
        err.to_string().contains("[openai_compatible.my-gateway]"),
        "error should include an actionable OpenAI-compatible BYOK example: {err}"
    );
    Ok(())
}
...
[openai]
api_key = "test-openai-direct-key"
...
discoverables = [
  ...
  { type = "plugin", id = "plugin_alpha@local-source" },
  { type = "connector", id = "   " }
]
"#,
    )
        ...
        Some(ToolSuggestConfig {
            discoverables: vec![
                ...
                ToolSuggestDiscoverable {
                    ...
                    id: "plugin_alpha@local-source".to_string(),
                },
                ToolSuggestDiscoverable {
                    kind: ToolSuggestDiscoverableType::Connector,
                    id: "   ".to_string(),
