...
use darwin_code_analytics::AppInvocation;
use darwin_code_analytics::InvocationType;
use darwin_code_analytics::build_track_events_context;
...
use crate::mcp_darwin_file::rewrite_mcp_tool_arguments_for_darwin_files;
...
use darwin_code_mcp::declared_darwin_file_input_param_names;
...
async fn execute_mcp_tool_call(
    ...
    let rewritten_arguments = rewrite_mcp_tool_arguments_for_darwin_files(
        sess,
        turn_context,
        arguments_value,
        metadata.and_then(|metadata| metadata.darwin_file_input_params.as_deref()),
    )
    .await?;
    let request_meta =
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    darwin_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_DARWIN_CODE_APPS_META_KEY: &str = "_darwin_code_apps";
const MCP_TOOL_DARWIN_OUTPUT_TEMPLATE_META_KEY: &str = "darwin/outputTemplate";
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    Some(McpToolApprovalMetadata {
        ...
        darwin_code_apps_meta: tool_info
            ...
            .cloned(),
        darwin_file_input_params: Some(declared_darwin_file_input_param_names(
...
fn get_mcp_app_resource_uri(
    ...
    meta.and_then(|meta| {
        meta.get("ui")
            ...
            .or_else(|| {
                meta.get(MCP_TOOL_DARWIN_OUTPUT_TEMPLATE_META_KEY)
                    .and_then(serde_json::Value::as_str)
            })
            .map(str::to_string)
    })
...
async fn persist_darwin_code_app_tool_approval(
    ...
    // App-tool approvals may still be remembered in-session, but DarwinCode
