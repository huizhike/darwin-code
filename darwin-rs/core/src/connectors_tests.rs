...
use darwin_code_config::types::AppsDefaultConfig;
use darwin_code_connectors::filter::filter_disallowed_connectors;
use darwin_code_connectors::filter::filter_tool_suggest_discoverable_connectors;
use darwin_code_connectors::merge::merge_connectors;
use darwin_code_connectors::merge::plugin_connector_to_app_info;
use darwin_code_connectors::metadata::connector_install_url;
use darwin_code_connectors::metadata::connector_mention_slug;
use darwin_code_connectors::metadata::sanitize_name;
...
fn darwin_code_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{DARWIN_CODE_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| DARWIN_CODE_APPS_MCP_SERVER_NAME.to_string());
...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    ...
    let tools = HashMap::from([
        ...
        (
            "mcp__darwin_code_apps__darwin_hidden".to_string(),
            darwin_code_app_tool(
                "darwin_hidden",
...
#[tokio::test]
async fn external_requirements_disable_connector_overrides_user_apps_config() {
    let darwin_code_home = tempdir().expect("tempdir should succeed");
    std::fs::write(
...
#[tokio::test]
async fn external_requirements_disable_connector_applies_without_user_apps_table() {
    let darwin_code_home = tempdir().expect("tempdir should succeed");
    std::fs::write(darwin_code_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(darwin_code_home.path().join(CONFIG_TOML_FILE))
            .expect("abs path");
    let mut config = ConfigBuilder::default()
        .darwin_code_home(darwin_code_home.path().to_path_buf())
        .fallback_cwd(Some(darwin_code_home.path().to_path_buf()))
...
#[test]
fn filter_disallowed_connectors_filters_darwin_prefix() {
    let filtered = filter_disallowed_connectors(
        vec![
            app("connector_darwin_foo"),
            app("connector_darwin_bar"),
            app("gamma"),
        ],
        "darwin_code_cli",
    );
...
#[test]
fn first_party_chat_originator_filters_target_and_darwin_prefixed_connectors() {
    let filtered = filter_disallowed_connectors(
        vec![
            app("connector_darwin_foo"),
            app("asdk_app_6938a94a61d881918ef32cb999ff937c"),
...
discoverables = [
  ...
  { type = "plugin", id = "slack@local-source" },
  { type = "connector", id = "   " }
]
"#,
    )
