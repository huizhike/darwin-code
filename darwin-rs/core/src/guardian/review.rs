...
pub(super) async fn run_guardian_review_session(
    ...
    let preferred_reasoning_effort = |supports_low: bool, fallback| {
        if supports_low {
            Some(darwin_code_protocol::model_metadata::ReasoningEffort::Low)
        } else {
            fallback
        }
    };
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == darwin_code_protocol::model_metadata::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == darwin_code_protocol::model_metadata::ReasoningEffort::Low
                }),
