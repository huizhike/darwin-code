...
pub fn apply_git_patch(req: &ApplyGitRequest) -> io::Result<ApplyGitResult> {
    ...
    if let Ok(cfg) = std::env::var("DARWIN_CODE_APPLY_GIT_CFG") {
...
mod tests {
    ...
    fn init_repo() -> tempfile::TempDir {
        ...
        let _ = run(root, &["git", "config", "user.email", "darwin@example.com"]);
        let _ = run(root, &["git", "config", "user.name", "DarwinCode"]);
        dir
    }
