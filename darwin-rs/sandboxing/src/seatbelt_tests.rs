...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/darwin-code-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.darwin".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/darwin-code-readable");
    let unreadable = absolute_path("/tmp/darwin-code-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
    ...
    assert!(
        args.iter()
            .any(|arg| arg.starts_with("-DWRITABLE_ROOT_0_EXCLUDED_")
                && arg.ends_with("/workspace/.darwin")),
        "expected proactive .darwin carveout for cwd root: {args:#?}",
    );
}
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/darwin-code-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root =
        normalize_path_for_sandbox(Path::new("/tmp/darwin-code-browser-use"))
...
fn unix_socket_dir_params_use_stable_param_names() {
    let params = unix_socket_dir_params(&SandboxNetworkPolicyInputs {
        unix_domain_socket_policy: UnixDomainSocketPolicy::Restricted {
            allowed: vec![absolute_path("/tmp/b.sock"), absolute_path("/tmp/a.sock")],
        },
        ..SandboxNetworkPolicyInputs::default()
    });
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_darwin_code_subpaths() {
    // Create a temporary workspace with two writable roots: one containing
    // top-level .git and .darwin directories and one without them.
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
        vulnerable_root_canonical,
        dot_git_canonical,
        dot_darwin_code_canonical,
        empty_root,
    ...
    // write to .darwin/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected cwd .darwin carveout in policy:\n{policy_text}",
    );
    assert!(
        policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_0")
            && policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_1"),
        "expected explicit writable root .git/.darwin carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_darwin_code_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .darwin/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_darwin_code_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .darwin is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_darwin_code_creation_with_exact_and_descendant_carveouts()
 {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_darwin = repo_root.join(".darwin");
    let config_toml = dot_darwin.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_darwin.to_string_lossy().as_ref(),
    ...
    assert!(
        ...
        "expected exact .darwin carveout in policy:\n{policy_text}"
    );
    assert!(
        policy_text.contains("(require-not (subpath (param \"WRITABLE_ROOT_0_EXCLUDED_1\")))"),
        "expected descendant .darwin carveout in policy:\n{policy_text}"
    );
}
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // `.darwin` checks are done properly for cwd.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: Default::default(),
        network_access: false,
    ...
    // - write access to WRITABLE_ROOT_0 (but not its .git or .darwin), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
struct PopulatedTmp {
    /// Path containing a .git and .darwin subfolder.
    ...
    /// ran `git commit` themselves, or modified .darwin/config.toml to
    ...
    dot_darwin_code_canonical: PathBuf,

    /// Path without .git or .darwin subfolders.
    empty_root: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".darwin")).expect("create .darwin");
    fs::write(
        vulnerable_root.join(".darwin").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .darwin/config.toml");
    ...
    let dot_darwin_code_canonical = vulnerable_root_canonical.join(".darwin");
