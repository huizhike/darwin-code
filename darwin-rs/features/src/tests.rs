...
fn external_migration_is_experimental_and_disabled_by_default() {
    ...
    assert_eq!(
        ...
        Some(
            "Show a startup prompt when DarwinCode detects migratable external agent config for this machine or project."
        )
    );
    assert_eq!(stage.experimental_announcement(), None);
