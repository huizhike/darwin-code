...
pub(super) fn test_session_telemetry(_config: &Config, model: &str) -> SessionTelemetry {
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget::new_with_op_target(
        ChatWidgetInit {
            ...
            model_catalog,
            feedback: darwin_code_feedback::DarwinCodeFeedback::new(),
            is_first_run: false,
...
fn test_model_info(slug: &str, priority: i32, supports_fast_mode: bool) -> ModelInfo {
    let additional_speed_tiers = if supports_fast_mode {
        vec![darwin_code_protocol::model_metadata::SPEED_TIER_FAST]
    } else {
        Vec::new()
    };
