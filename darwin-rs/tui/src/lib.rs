...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        darwin_code_feedback::DarwinCodeFeedback::new(),
        /*log_db*/ None,
        environment_manager,
    )
    .await?;
...
async fn lookup_session_target_by_name_with_app_server(
    ...
    loop {
        ...
        if response.next_cursor.is_none() {
            ...
            return Ok(find_thread_meta_by_name_str(darwin_code_home, name)
                .await?
                .map(|(path, session_meta)| resume_picker::SessionTarget {
                    path: Some(path),
...
pub async fn run_main(
    ...
    let feedback = darwin_code_feedback::DarwinCodeFeedback::new();
...
async fn run_ratatui_app(
    ...
    let should_show_onboarding = should_show_onboarding(should_show_trust_screen_flag);
...
fn should_show_onboarding(show_trust_screen: bool) -> bool {
    show_trust_screen
}
