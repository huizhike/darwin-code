    ...
    pub(crate) fn new() -> Self {
        ...
        tokio::spawn(async move {
            ...
            while let Some(input) = receiver.recv().await {
                ...
                send_track_events(events).await;
            }
        });
        Self {
            sender,
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::DarwinCodeCompactionEvent) {
...
async fn send_track_events(events: Vec<TrackEventRequest>) {
    if events.is_empty() {
        return;
    }

    // BYOK-only DarwinCode does not upload proprietary hosted analytics events from
    // CLI/core/app-server.  Keep the reducer and in-process tracking path intact for
    // callers/tests, but intentionally drop outbound events at this boundary.
    tracing::debug!(
        event_count = events.len(),
        "dropping analytics events in BYOK-only local runtime"
    );
}
