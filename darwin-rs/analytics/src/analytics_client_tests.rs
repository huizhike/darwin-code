...
use crate::events::DarwinCodeAppMentionedEventRequest;
use crate::events::DarwinCodeAppServerClientMetadata;
use crate::events::DarwinCodeAppUsedEventRequest;
use crate::events::DarwinCodeCompactionEventRequest;
use crate::events::DarwinCodeHookRunEventRequest;
use crate::events::DarwinCodePluginEventRequest;
use crate::events::DarwinCodePluginUsedEventRequest;
use crate::events::DarwinCodeRuntimeMetadata;
use crate::events::DarwinCodeTurnEventRequest;
use crate::events::ThreadInitializedEvent;
use crate::events::ThreadInitializedEventParams;
use crate::events::TrackEventRequest;
use crate::events::darwin_code_app_metadata;
use crate::events::darwin_code_hook_run_metadata;
use crate::events::darwin_code_plugin_metadata;
use crate::events::darwin_code_plugin_used_metadata;
...
use crate::facts::DarwinCodeCompactionEvent;
...
use darwin_code_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use darwin_code_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use darwin_code_app_server_protocol::NonSteerableTurnKind;
use darwin_code_app_server_protocol::RequestId;
use darwin_code_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use darwin_code_app_server_protocol::ServerNotification;
use darwin_code_app_server_protocol::SessionSource as AppServerSessionSource;
...
use darwin_code_app_server_protocol::ThreadStatus as AppServerThreadStatus;
...
use darwin_code_app_server_protocol::TurnStatus as AppServerTurnStatus;
...
use darwin_code_client::DEFAULT_ORIGINATOR;
...
fn sample_app_server_client_metadata() -> DarwinCodeAppServerClientMetadata {
    DarwinCodeAppServerClientMetadata {
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
        client_name: Some("darwin-code-tui".to_string()),
...
fn sample_runtime_metadata() -> DarwinCodeRuntimeMetadata {
    DarwinCodeRuntimeMetadata {
        darwin_code_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str, request_id: i64) -> ClientResponse {
    ClientResponse::TurnStart {
        request_id: RequestId::Integer(request_id),
        response: darwin_code_app_server_protocol::TurnStartResponse {
            turn: Turn {
...
fn sample_turn_completed_notification(
    ...
    darwin_code_error_info: Option<darwin_code_app_server_protocol::DarwinCodeErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: darwin_code_error_info.map(|darwin_code_error_info| AppServerTurnError {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "darwin-code-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "darwin-code-tui".to_string(),
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.darwin/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".darwin/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.darwin/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/darwin/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.darwin/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(DarwinCodeAppMentionedEventRequest {
        event_type: "darwin_code_app_mentioned",
        event_params: darwin_code_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(DarwinCodeAppUsedEventRequest {
        event_type: "darwin_code_app_used",
        event_params: darwin_code_app_metadata(
            &tracking,
            AppInvocation {
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(DarwinCodeCompactionEventRequest {
        event_type: "darwin_code_compaction_event",
        event_params: crate::events::darwin_code_compaction_event_params(
            DarwinCodeCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "darwin-code-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "darwin_code_rs_version": "0.1.0",
                    "runtime_os": "macos",
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "darwin_code_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            app_server_client: DarwinCodeAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("darwin-code-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
            ...
            runtime: DarwinCodeRuntimeMetadata {
                darwin_code_rs_version: "0.1.0".to_string(),
                runtime_os: "macos".to_string(),
                runtime_os_version: "15.3.1".to_string(),
                runtime_arch: "aarch64".to_string(),
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                runtime: DarwinCodeRuntimeMetadata {
                    darwin_code_rs_version: "0.99.0".to_string(),
    ...
    assert_eq!(payload[0]["event_type"], "darwin_code_thread_initialized");
    assert_eq!(
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["darwin_code_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        darwin_code_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "darwin_code_compaction_event");
...
fn subagent_thread_started_thread_spawn_serializes_parent_thread_id() {
    let parent_thread_id =
        darwin_code_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(DarwinCodePluginUsedEventRequest {
        event_type: "darwin_code_plugin_used",
        event_params: darwin_code_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(DarwinCodePluginEventRequest {
        event_type: "darwin_code_plugin_installed",
        event_params: darwin_code_plugin_metadata(sample_plugin_metadata()),
    });
...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(DarwinCodeHookRunEventRequest {
        event_type: "darwin_code_hook_run",
        event_params: darwin_code_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(darwin_code_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(darwin_code_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let unknown = serde_json::to_value(darwin_code_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(darwin_code_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: darwin_code_protocol::protocol::SkillScope::User,
                    skill_path,
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "darwin_code_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "darwin_code_app_mentioned");
    assert_eq!(payload[1]["event_type"], "darwin_code_app_used");
    assert_eq!(payload[2]["event_type"], "darwin_code_plugin_used");
}
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "darwin_code_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "plugin_name": "sample",
                "source_name": "test",
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(DarwinCodeTurnEventRequest {
        event_type: "darwin_code_turn_event",
        event_params: crate::events::DarwinCodeTurnEventParams {
            thread_id: "thread-2".to_string(),
            turn_id: "turn-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "darwin_code_cli_rs",
                    "client_name": "darwin-code-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("darwin_code_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["app_server_client"]["product_client_id"],
        json!("darwin-code-tui")
    );
    assert_eq!(
        payload["event_params"]["runtime"]["darwin_code_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("darwin_code_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(payload["event_params"]["turn_id"], json!("turn-2"));
    assert_eq!(
        payload["event_params"]["app_server_client"],
        json!({
            "product_client_id": "darwin-code-tui",
            "client_name": "darwin-code-tui",
            "client_version": "1.0.0",
            "rpc_transport": "stdio",
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(darwin_code_app_server_protocol::DarwinCodeErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
