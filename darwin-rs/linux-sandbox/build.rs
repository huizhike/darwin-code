...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=DARWIN_CODE_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=DARWIN_CODE_SKIP_VENDORED_BWRAP");
    ...
    if target_os != "linux" || env::var_os("DARWIN_CODE_SKIP_VENDORED_BWRAP").is_some() {
        return;
    }
...
fn try_build_vendored_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built at darwin build-time"
"#,
    )
...
///
/// Priority:
/// 1. `DARWIN_CODE_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `darwin-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("DARWIN_CODE_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "DARWIN_CODE_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set DARWIN_CODE_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under darwin-rs/vendor.",
        vendor_dir.display()
    ))
}
