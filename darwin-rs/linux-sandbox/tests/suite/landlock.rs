...
async fn run_cmd_result_with_policies(
    ...
    let darwin_code_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(DarwinCodeErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(DarwinCodeErr::Sandbox(SandboxErr::Timeout { .. })) => true,
...
fn expect_denied(
    ...
    match result {
        ...
        Err(DarwinCodeErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_darwin-code-linux-sandbox");
    let darwin_code_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
...
async fn sandbox_blocks_curl() {
    assert_network_blocked(&["curl", "-I", "http://example.com"]).await;
}

#[tokio::test]
async fn sandbox_blocks_wget() {
    assert_network_blocked(&["wget", "-qO-", "http://example.com"]).await;
}
...
#[tokio::test]
async fn sandbox_blocks_git_and_darwin_code_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_darwin = tmpdir.path().join(".darwin");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_darwin).expect("create .darwin");

    let git_target = dot_git.join("config");
    let darwin_code_target = dot_darwin.join("config.toml");
    ...
    let darwin_code_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", darwin_code_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".darwin write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(darwin_code_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_darwin_code_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-darwin");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_darwin = tmpdir.path().join(".darwin");
    symlink(&decoy, &dot_darwin).expect("create .darwin symlink");
    ...
    let darwin_code_output = expect_denied(
        ...
        .await,
        ".darwin symlink replacement should be denied",
    );
    assert_ne!(darwin_code_output.exit_code, 0);
}
...
async fn sandbox_blocks_getent() {
    assert_network_blocked(&["getent", "ahosts", "example.com"]).await;
}
