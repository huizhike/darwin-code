# darwin-code-linux-sandbox

This crate is responsible for producing:

- a `darwin-code-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Darwin Code CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `darwin-code-exec` CLI can check if its arg0 is `darwin-code-linux-sandbox` and, if so, execute as if it were `darwin-code-linux-sandbox`
  - this should also be true of the `darwin` multitool CLI

On Linux, Darwin Code prefers the first `bwrap` found on `PATH`
...
binary.
Darwin Code also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the vendored helper. Darwin Code surfaces the same startup warning
...
it cannot create the required user namespaces, so Darwin Code rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the vendored bubblewrap
  path.
- If `bwrap` is missing, Darwin Code also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Darwin Code surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Darwin Code rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.darwin`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface still uses legacy names like `darwin debug landlock`.
