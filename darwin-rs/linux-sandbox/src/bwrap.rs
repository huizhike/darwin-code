...
//! - sensitive subpaths such as `.git` and `.darwin` remain read-only even when
...
mod tests {
    ...
    fn restricted_policy_chdirs_to_canonical_command_cwd() {
        ...
        assert!(
            args.args
                .windows(3)
                .any(|window| { window == ["--ro-bind", canonical_sandbox_cwd.as_str(),] })
        );
        assert!(
            !args
                .args
        ...
        assert!(
            !args
                .args
                .windows(3)
                .any(|window| { window == ["--ro-bind", link_sandbox_cwd.as_str(),] })
        );
    }
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_darwin = temp_dir.path().join("real-darwin");
        let logical_darwin = logical_home.join(".darwin");
        let real_memories = real_darwin.join("memories");
        let logical_memories = logical_darwin.join("memories");
        ...
        std::os::unix::fs::symlink(&real_darwin, &logical_darwin)
            .expect("create symlinked darwin home");
        ...
        assert!(
            args.args
                .windows(3)
                .any(|window| { window == ["--bind", real_memories_str.as_str(),] })
        );
        assert!(
            !args
                .args
                .windows(3)
                .any(|window| { window == ["--bind", logical_memories_str.as_str(),] })
        );
    }
    ...
    fn restricted_read_only_uses_scoped_read_roots_instead_of_erroring() {
        ...
        assert!(
            args.args
                .windows(3)
                .any(|window| { window == ["--ro-bind", readable_root_str.as_str(),] })
        );
    }
    ...
    fn split_policy_reapplies_unreadable_carveouts_after_writable_binds() {
        ...
        assert!(
            args.args
                .windows(3)
                .any(|window| { window == ["--bind", writable_root_str.as_str(),] })
        );
        ...
        let writable_root_bind_index = args
            .args
            .windows(3)
            .position(|window| window == ["--bind", writable_root_str.as_str()])
    ...
    fn split_policy_reenables_writable_files_after_unreadable_parent() {
        ...
        let allowed_bind_index = args
            .args
            .windows(3)
            .position(|window| window == ["--bind", allowed_file_str.as_str()])
