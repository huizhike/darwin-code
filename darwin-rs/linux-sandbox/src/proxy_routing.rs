...
const PROXY_SOCKET_DIR_PREFIX: &str = "darwin-code-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(darwin_code_home) = std::env::var_os("DARWIN_CODE_HOME") {
        let candidate = PathBuf::from(darwin_code_home).join("tmp");
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("darwin-code-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("darwin-code-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("darwin-code-linux-sandbox-proxy-x"),
            None
        );
