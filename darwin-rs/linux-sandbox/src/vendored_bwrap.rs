...
darwin-code-linux-sandbox should always compile vendored bubblewrap on Linux targets.
Notes:
- ensure the target OS is Linux
- libcap headers must be available via pkg-config
- bubblewrap sources expected at darwin-rs/vendor/bubblewrap (default)"#
        );
    }
