//! Persist DarwinCode session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
///
/// ```ignore
/// $ jq -C . ~/.darwin/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.darwin/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided DarwinCode home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let darwin_code_home = config.darwin_code_home();
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = darwin_code_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(darwin_code_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(darwin_code_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.darwin/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.darwin_code_home().to_path_buf();
