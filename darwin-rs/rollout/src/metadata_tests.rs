...
fn test_config(darwin_code_home: PathBuf) -> RolloutConfig {
    RolloutConfig {
        sqlite_home: darwin_code_home.clone(),
        cwd: darwin_code_home.clone(),
        darwin_code_home,
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let darwin_code_home = dir.path().to_path_buf();
    ...
    let first_watermark =
        backfill_watermark_for_path(darwin_code_home.as_path(), first_path.as_path());
    ...
    let config = test_config(darwin_code_home.clone());
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let darwin_code_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = darwin_code_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = darwin_code_home.join("sessions");
