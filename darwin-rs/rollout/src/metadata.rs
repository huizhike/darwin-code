...
pub(crate) async fn backfill_sessions(
    ...
    let sessions_root = config.darwin_code_home().join(SESSIONS_SUBDIR);
    let archived_root = config.darwin_code_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.darwin_code_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(darwin_code_home: &Path, path: &Path) -> String {
    path.strip_prefix(darwin_code_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
