...
pub async fn open_if_present(
    ...
    let db_path = darwin_code_state::state_db_path(darwin_code_home);
    ...
    .await
    .ok()?;
    require_backfill_complete(runtime, darwin_code_home).await
}
...
pub async fn list_thread_ids_db(
    ...
    let ctx = context?;
    if ctx.darwin_code_home() != darwin_code_home {
        warn!(
            "state db darwin_code_home mismatch: expected {}, got {}",
            ctx.darwin_code_home().display(),
            darwin_code_home.display()
        );
    }
