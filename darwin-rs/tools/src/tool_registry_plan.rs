...
use darwin_code_protocol::model_metadata::ApplyPatchToolType;
...
pub fn build_tool_registry_plan(
        ...
        && (params.deferred_mcp_tools.is_some() || !deferred_dynamic_tools.is_empty())
    {
        ...
        if !deferred_dynamic_tools.is_empty() {
            search_source_infos.push(ToolSearchSourceInfo {
                ...
                description: Some("Tools provided by the current DarwinCode thread.".to_string()),
            });
        }
            ...
            for tool in entries {
                match mcp_tool_to_responses_api_tool(&tool.name, tool.tool) {
                    ...
                    Err(error) => {
                        let tool_name = &tool.name;
                        tracing::error!(
                            "Failed to convert `{tool_name}` MCP tool to provider tool: {error:?}"
                        );
                    }
                }
            }
    ...
    for tool in params.dynamic_tools {
        match dynamic_tool_to_responses_api_tool(tool) {
            ...
            Err(error) => {
                tracing::error!(
                    "Failed to convert dynamic tool {:?} to provider tool: {error:?}",
                    tool.name
                );
            }
        }
