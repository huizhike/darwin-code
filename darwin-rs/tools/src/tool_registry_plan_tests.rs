...
const DARWIN_CODE_APPS_MCP_SERVER_NAME: &str = "darwin_code_apps";
...
#[test]
fn test_full_toolset_specs_for_gpt5_darwin_code_unified_exec_web_search() {
...
fn search_tool_description_lists_each_mcp_source_once() {
    ...
    assert!(handlers.contains(&ToolHandlerSpec {
        name: ToolName::namespaced("mcp__darwin_code_apps__calendar", "_create_event"),
...
fn search_tool_registers_for_deferred_dynamic_tools() {
    ...
    assert!(
        description.contains("- Dynamic tools: Tools provided by the current DarwinCode thread.")
    );
