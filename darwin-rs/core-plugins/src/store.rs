...
use darwin_code_plugin::validate_plugin_segment;
...
impl PluginStore {
    pub fn new(darwin_code_home: PathBuf) -> Self {
        Self {
            root: AbsolutePathBuf::try_from(darwin_code_home.join(PLUGINS_CACHE_DIR))
    ...
    pub fn install_with_version(
        ...
        if plugin_name != plugin_id.plugin_name {
            return Err(PluginStoreError::Invalid(format!(
                "plugin.json name `{plugin_name}` does not match plugin source name `{}`",
