...
#[test]
fn install_copies_plugin_into_default_source() {
    ...
    assert!(
        installed_path
            .join(".darwin-code-plugin/plugin.json")
            .is_file()
    );
...
fn install_uses_manifest_name_for_destination_and_key() {
    ...
    let plugin_id = PluginId::new("manifest-name".to_string(), "source".to_string()).unwrap();
    ...
    assert_eq!(
        ...
        PluginInstallResult {
            ...
            installed_path: AbsolutePathBuf::try_from(
                tmp.path().join("plugins/cache/source/manifest-name/local"),
            )
            .unwrap(),
        }
    );
...
fn install_with_version_uses_requested_cache_version() {
    let tmp = tempdir().unwrap();
    write_plugin(tmp.path(), "sample-plugin", "sample-plugin");
    let plugin_id = PluginId::new("sample-plugin".to_string(), "local-source".to_string()).unwrap();
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/local-source/sample-plugin/{plugin_version}"
    ));
    assert_eq!(
        result,
        PluginInstallResult {
...
fn install_uses_manifest_version_when_present() {
    let tmp = tempdir().unwrap();
    write_plugin_with_version(tmp.path(), "sample-plugin", Some("1.2.3-beta+7"));
    let plugin_id = PluginId::new("sample-plugin".to_string(), "debug".to_string()).unwrap();
...
fn plugin_root_rejects_path_separators_in_key_segments() {
    ...
    assert_eq!(
        err.to_string(),
        "invalid plugin source name: only ASCII letters, digits, `_`, and `-` are allowed in `sample@../../etc`"
    );
}
...
#[test]
fn install_rejects_source_names_with_path_separators() {
    let err = PluginId::new("sample-plugin".to_string(), "../../etc".to_string()).unwrap_err();

    assert_eq!(
        err.to_string(),
        "invalid plugin source name: only ASCII letters, digits, `_`, and `-` are allowed"
    );
}

#[test]
fn install_rejects_manifest_names_that_do_not_match_plugin_id_name() {
    let tmp = tempdir().unwrap();
    write_plugin(tmp.path(), "source-dir", "manifest-name");
    ...
    assert_eq!(
        err.to_string(),
        "plugin.json name `manifest-name` does not match plugin source name `different-name`"
    );
}
