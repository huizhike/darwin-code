...
use crate::darwin_code_tool_config::DarwinCodeToolCallParam;
use crate::darwin_code_tool_config::DarwinCodeToolCallReplyParam;
use crate::darwin_code_tool_config::create_tool_for_darwin_code_tool_call_param;
use crate::darwin_code_tool_config::create_tool_for_darwin_code_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_darwin_code_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_darwin_code_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            ...
            title: Some("DarwinCode".to_string()),
        ...
        // Preserve DarwinCode's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert(
                ...
                json!(get_darwin_code_user_agent()),
            );
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_darwin_code_tool_call_param(),
                create_tool_for_darwin_code_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "darwin" => self.handle_tool_call_darwin(id, arguments).await,
            "darwin-code-reply" => {
                self.handle_tool_call_darwin_code_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_darwin(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<DarwinCodeToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to load DarwinCode configuration from overrides: {e}"
                            ))],
                            structured_content: None,
                            is_error: Some(true),
                            meta: None,
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for DarwinCode tool: {e}"
                        ))],
                        structured_content: None,
                        is_error: Some(true),
                        meta: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for darwin tool-call; the `prompt` field is required.",
                    )],
                    structured_content: None,
                    is_error: Some(true),
                    meta: None,
        ...
        let running_requests_id_to_darwin_code_uuid =
            self.running_requests_id_to_darwin_code_uuid.clone();

        // Spawn an async task to handle the DarwinCode session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the DarwinCode session and stream events back to the client.
            crate::darwin_code_tool_runner::run_darwin_code_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_darwin_code_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_darwin_code_session_reply(
        &self,
        ...
        // parse arguments
        let darwin_code_tool_call_reply_param: DarwinCodeToolCallReplyParam = match arguments {
            Some(json_val) => {
                match serde_json::from_value::<DarwinCodeToolCallReplyParam>(json_val) {
                    Ok(params) => params,
                    Err(e) => {
                        tracing::error!(
                            "Failed to parse DarwinCode tool call reply parameters: {e}"
                        );
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to parse configuration for DarwinCode tool: {e}"
            ...
            None => {
                tracing::error!(
                    "Missing arguments for darwin-code-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for darwin-code-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match darwin_code_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let darwin = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::darwin_code_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = darwin_code_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_darwin_code_uuid =
                running_requests_id_to_darwin_code_uuid.clone();

            async move {
                crate::darwin_code_tool_runner::run_darwin_code_tool_session_reply(
                    thread_id,
                    darwin,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_darwin_code_uuid.lock().await;
        ...
        // Obtain the DarwinCode thread from the server.
        let darwin_code_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to DarwinCode.
        if let Err(e) = darwin_code_arc
            ...
            .await
        {
            tracing::error!("Failed to submit interrupt to DarwinCode: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_darwin_code_uuid
            .lock()
            .await
            .remove(&request_id);
    }
