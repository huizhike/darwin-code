//! Asynchronous worker that executes a **DarwinCode** tool-call inside a spawned
...
use darwin_code_core::config::Config as DarwinCodeConfig;
...
/// To adhere to MCP `tools/call` response format, include the DarwinCode
...
/// Run a complete DarwinCode session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_darwin_code_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: DarwinCodeConfig,
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!(
                    "Failed to start DarwinCode session: {e}"
                ))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the DarwinCode submission so that
    ...
    run_darwin_code_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_darwin_code_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_darwin_code_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<DarwinCodeThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(darwin_code_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("DarwinCode runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
