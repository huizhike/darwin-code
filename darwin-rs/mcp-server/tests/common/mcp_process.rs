...
impl McpProcess {
    pub async fn new(darwin_code_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(darwin_code_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = darwin_code_utils_cargo_bin::cargo_bin("darwin-code-mcp-server")
    ...
    pub async fn send_darwin_code_tool_call(
        &mut self,
        params: DarwinCodeToolCallParam,
    ) -> anyhow::Result<i64> {
        let darwin_code_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "darwin".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(darwin_code_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "darwin/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "darwin/event" {
