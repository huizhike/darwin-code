...
use darwin_code_mcp_server::DarwinCodeToolCallParam;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "darwin" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let darwin_code_request_id = mcp_process
        .send_darwin_code_tool_call(DarwinCodeToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            darwin_code_request_id.to_string(),
            params.darwin_code_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `darwin` tool call completes and that the file was created.
    let darwin_code_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(darwin_code_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(darwin_code_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    darwin_code_mcp_tool_call_id: String,
    darwin_code_event_id: String,
    thread_id: darwin_code_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow DarwinCode to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let darwin_code_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        darwin_code_elicitation: "exec-approval".to_string(),
        darwin_code_mcp_tool_call_id,
        darwin_code_event_id,
        darwin_code_command: command,
        darwin_code_cwd: workdir.to_path_buf(),
        darwin_code_call_id: "call1234".to_string(),
        darwin_code_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "darwin" tool request that will trigger the apply_patch command
    let darwin_code_request_id = mcp_process
        .send_darwin_code_tool_call(DarwinCodeToolCallParam {
    ...
    // Verify the original `darwin` tool call completes
    let darwin_code_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(darwin_code_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_darwin_code_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = darwin_code_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn darwin_code_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `darwin mcp` with a specific config.toml.
    let darwin_code_home = TempDir::new()?;
    create_config_toml(darwin_code_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(darwin_code_home.path()).await?;
    ...
    .await??;
    assert_eq!(darwin_code_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(
        darwin_code_response.id,
        RequestId::Number(darwin_code_request_id)
    );
    assert_eq!(
        darwin_code_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": darwin_code_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("darwin tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow DarwinCode to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        darwin_code_elicitation: "patch-approval".to_string(),
        darwin_code_mcp_tool_call_id,
        darwin_code_event_id,
        darwin_code_reason: reason,
        darwin_code_grant_root: grant_root,
        darwin_code_changes: changes,
        darwin_code_call_id: "call1234".to_string(),
    })?;
...
async fn create_mcp_process(responses: Vec<String>) -> anyhow::Result<McpHandle> {
    ...
    Ok(McpHandle {
        process: mcp_process,
        server,
        dir: darwin_code_home,
    })
}

/// Create a DarwinCode config that uses the mock server as the model provider.
