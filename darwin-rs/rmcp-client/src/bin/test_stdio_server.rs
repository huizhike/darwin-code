...
const MEMO_URI: &str = "memo://darwin/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "darwin/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of DarwinCode TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (DarwinCode TUI):
    /// - Build this binary: `cargo build -p darwin-code-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `darwin mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in DarwinCode TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://darwin/{slug}".to_string(),
            name: "darwin-code-memo".to_string(),
            title: Some("DarwinCode Memo".to_string()),
            description: Some(
                "Template for memo://darwin/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise DarwinCode TUI handling of MCP image outputs.
///
