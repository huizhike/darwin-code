...
fn server_overloaded_maps_to_protocol() {
    let err = DarwinCodeErr::ServerOverloaded;
    assert_eq!(
        err.to_darwin_code_protocol_error(),
        DarwinCodeErrorInfo::ServerOverloaded
    );
}
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = DarwinCodeErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = DarwinCodeErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.darwin_code_error_info,
        Some(DarwinCodeErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_enterprise_cbp_usage_based_plan() {
    let err = UsageLimitReachedError {
        plan_type: Some(PlanType::EnterpriseCbpUsageBased),
        resets_at: None,
        rate_limits: Some(Box::new(rate_limit_snapshot())),
        promo_message: None,
    };
...
fn usage_limit_reached_error_formats_default_for_other_plans() {
    let err = UsageLimitReachedError {
        plan_type: Some(PlanType::Enterprise),
        resets_at: None,
        rate_limits: Some(Box::new(rate_limit_snapshot())),
        promo_message: None,
    };
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_darwin_code_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        ...
        let expected = format!(
            "You've hit your usage limit for darwin_code_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using DarwinCode, start a free trial of <PLAN> today".to_string(),
            ),
        };
        let expected = format!(
            "You've hit your usage limit. To continue using DarwinCode, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
