...
pub type Result<T> = std::result::Result<T, DarwinCodeErr>;
...
pub enum DarwinCodeErr {
    ...
    #[error(
        "DarwinCode ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `DarwinCodeError::Spawn` variant.
    ...
    #[error("darwin-code-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for DarwinCodeErr {
    fn from(_: CancelErr) -> Self {
        DarwinCodeErr::TurnAborted
    }
}
...
impl DarwinCodeErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            DarwinCodeErr::TurnAborted
            | DarwinCodeErr::Interrupted
            | DarwinCodeErr::EnvVar(_)
            | DarwinCodeErr::Fatal(_)
            | DarwinCodeErr::UsageNotIncluded
            | DarwinCodeErr::QuotaExceeded
            | DarwinCodeErr::InvalidImageRequest()
            | DarwinCodeErr::InvalidRequest(_)
            | DarwinCodeErr::UnsupportedOperation(_)
            | DarwinCodeErr::Sandbox(_)
            | DarwinCodeErr::LandlockSandboxExecutableNotProvided
            | DarwinCodeErr::RetryLimit(_)
            | DarwinCodeErr::ContextWindowExceeded
            | DarwinCodeErr::ThreadNotFound(_)
            | DarwinCodeErr::AgentLimitReached { .. }
            | DarwinCodeErr::Spawn
            | DarwinCodeErr::SessionConfiguredNotFirstEvent
            | DarwinCodeErr::UsageLimitReached(_)
            | DarwinCodeErr::ServerOverloaded => false,
            DarwinCodeErr::Stream(..)
            | DarwinCodeErr::Timeout
            | DarwinCodeErr::UnexpectedStatus(_)
            | DarwinCodeErr::ResponseStreamFailed(_)
            | DarwinCodeErr::ConnectionFailed(_)
            | DarwinCodeErr::InternalServerError
            | DarwinCodeErr::InternalAgentDied
            | DarwinCodeErr::Io(_)
            | DarwinCodeErr::Json(_)
            | DarwinCodeErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            DarwinCodeErr::LandlockRuleset(_) | DarwinCodeErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<DarwinCodeErr>()` checks continue to compile
    ...
    pub fn to_darwin_code_protocol_error(&self) -> DarwinCodeErrorInfo {
        match self {
            DarwinCodeErr::ContextWindowExceeded => DarwinCodeErrorInfo::ContextWindowExceeded,
            DarwinCodeErr::UsageLimitReached(_)
            | DarwinCodeErr::QuotaExceeded
            | DarwinCodeErr::UsageNotIncluded => DarwinCodeErrorInfo::UsageLimitExceeded,
            DarwinCodeErr::ServerOverloaded => DarwinCodeErrorInfo::ServerOverloaded,
            DarwinCodeErr::RetryLimit(_) => DarwinCodeErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            DarwinCodeErr::ConnectionFailed(_) => DarwinCodeErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            DarwinCodeErr::ResponseStreamFailed(_) => {
                DarwinCodeErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            DarwinCodeErr::SessionConfiguredNotFirstEvent
            | DarwinCodeErr::InternalServerError
            | DarwinCodeErr::InternalAgentDied => DarwinCodeErrorInfo::InternalServerError,
            DarwinCodeErr::UnsupportedOperation(_)
            | DarwinCodeErr::ThreadNotFound(_)
            | DarwinCodeErr::AgentLimitReached { .. } => DarwinCodeErrorInfo::BadRequest,
            DarwinCodeErr::Sandbox(_) => DarwinCodeErrorInfo::SandboxError,
            _ => DarwinCodeErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            darwin_code_error_info: Some(self.to_darwin_code_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            DarwinCodeErr::RetryLimit(err) => Some(err.status),
            DarwinCodeErr::UnexpectedStatus(err) => Some(err.status),
            DarwinCodeErr::ConnectionFailed(err) => err.source.status(),
            DarwinCodeErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("darwin")
        {
            return write!(
                f,
...
pub fn get_error_message_ui(e: &DarwinCodeErr) -> String {
    let message = match e {
        DarwinCodeErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        DarwinCodeErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
