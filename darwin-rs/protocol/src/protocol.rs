//! Defines the protocol for a DarwinCode session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for DarwinCode:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::darwin_code_thread::DarwinCodeThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by darwin-code-core; it does not
    /// involve the model.
    ...
    /// Request DarwinCode to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request DarwinCode to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down darwin instance.
    Shutdown,
...
/// running the command proposed by DarwinCode.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.darwin`, `.git`, notably `.git/hooks`) under
...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .darwin read-only to the agent by
    ...
    let top_level_darwin = writable_root.join(".darwin");
    if protect_missing_dot_darwin || top_level_darwin.as_path().is_dir() {
        subpaths.push(top_level_darwin);
    }
...
/// DarwinCode errors that we expose to clients.
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.darwin_code_error_info
            .as_ref()
            .is_none_or(DarwinCodeErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::DarwinCode),
...
mod tests {
    ...
    #[test]
    fn product_app_platform_uses_darwin_code_for_darwin_code_product() {
        assert_eq!(Product::DarwinCode.to_app_platform(), "darwin_code");
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_darwin() {
        assert_eq!(
            SessionSource::Cli.restriction_product(),
            Some(Product::DarwinCode)
        );
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("darwin".to_string()).restriction_product(),
            Some(Product::DarwinCode)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::DarwinCode]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".darwin")).expect("create .darwin");
        let canonical_cwd =
            darwin_code_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_darwin = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".darwin"))
            .expect("canonical .darwin");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_darwin.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_darwin =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".darwin"))
                .expect("canonical .darwin");
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_darwin.to_path_buf(),
