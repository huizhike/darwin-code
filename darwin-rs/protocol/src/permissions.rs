...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer DarwinCode degrades to warn-and-ignore
    /// instead of failing to load. DarwinCode 0.112.0 rejected unknown values here,
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            ...
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_darwin*/ true,
                ) {
            ...
            for writable_root in writable_roots {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    writable_root,
                    /*protect_missing_dot_darwin*/ false,
                ) {
                    append_default_read_only_path_if_no_explicit_rule(
                        &mut file_system_policy.entries,
                        protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let raw_writable_roots: Vec<&AbsolutePathBuf> = writable_entries
                ...
                .collect();
            let protect_missing_dot_darwin = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_darwin)
                    .into_iter()
            ...
            // Example: if `<root>/.darwin -> <root>/decoy`, bwrap must still see
            // `<root>/.darwin`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.darwin` so downstream sandboxes can still detect and mask
...
fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_darwin: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    for metadata_dir in [".darwin_code", ".darwin"] {
        let top_level_metadata = writable_root.join(metadata_dir);
        if protect_missing_dot_darwin || top_level_metadata.as_path().is_dir() {
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "DARWIN_CODE_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn writable_roots_proactively_protect_missing_dot_darwin() {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_darwin)
        );
    }

    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_darwin_code_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_darwin = expected_root.join(".darwin");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_darwin.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_darwin),
            "explicit .darwin rule should win over the default protected carveout"
        );
        assert!(policy.can_write_path_with_cwd(
            explicit_dot_darwin.join("config.toml").as_path(),
            cwd.path()
        ));
    }

    #[test]
    fn legacy_workspace_write_projection_blocks_missing_dot_darwin_code_writes() {
        let cwd = TempDir::new().expect("tempdir");
        let dot_darwin_code_config = cwd.path().join(".darwin").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_darwin_code_config, cwd.path()));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        let expected_dot_darwin = AbsolutePathBuf::from_absolute_path(
            std::env::current_dir()
                .expect("current dir")
                .join(relative_cwd)
                .join(".darwin"),
        )
        .expect("absolute dot darwin");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            read_only_access: ReadOnlyAccess::Restricted {
                include_platform_defaults: false,
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".darwin/config.toml"), relative_cwd,)
        );
    }
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let darwin_code_dir = real_root.join(".darwin");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&darwin_code_dir).expect("create .darwin");
        ...
        let expected_darwin = link_root.join(".darwin");
    ...
    fn current_working_directory_special_path_preserves_symlinked_cwd() {
        ...
        let expected_darwin = expected_root.join(".darwin");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-darwin");
        let dot_darwin = root.join(".darwin");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_darwin).expect("create .darwin symlink");
        ...
        .expect("absolute .darwin symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let darwin_code_dir = real_tmpdir.join(".darwin");
