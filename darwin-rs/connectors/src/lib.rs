...
use darwin_code_app_server_protocol::AppBranding;
use darwin_code_app_server_protocol::AppInfo;
use darwin_code_app_server_protocol::AppMetadata;
...
fn connector_install_url(name: &str, connector_id: &str) -> String {
    ...
    format!("darwin-code://apps/{slug}/{connector_id}")
}
...
mod tests {
    ...
    static CONNECTORS_CACHE_TEST_LOCK: std::sync::LazyLock<tokio::sync::Mutex<()>> =
        std::sync::LazyLock::new(|| tokio::sync::Mutex::new(()));

    fn clear_connector_cache_for_test() {
        let mut cache_guard = ALL_CONNECTORS_CACHE
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        *cache_guard = None;
    ...
    async fn list_all_connectors_uses_shared_cache() -> anyhow::Result<()> {
        let _guard = CONNECTORS_CACHE_TEST_LOCK.lock().await;
        clear_connector_cache_for_test();
    ...
    async fn list_all_connectors_merges_and_normalizes_directory_apps() -> anyhow::Result<()> {
        ...
        assert_eq!(
            ...
            Some("darwin-code://apps/alpha/alpha")
        );
        assert_eq!(
            connectors[0]
                .branding
