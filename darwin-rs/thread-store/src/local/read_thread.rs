...
async fn resolve_rollout_path(
    ...
    if include_archived {
        match find_thread_path_by_id_str(
            store.config.darwin_code_home.as_path(),
            &thread_id.to_string(),
        )
        .await
...
async fn read_thread_from_rollout_path(
    ...
    if let Ok(Some(title)) =
        find_thread_name_by_id(store.config.darwin_code_home.as_path(), &thread_id).await
    {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let name = match distinct_title(&metadata) {
        ...
        None => find_thread_name_by_id(store.config.darwin_code_home.as_path(), &metadata.id)
            .await
            .ok()
            .flatten(),
    };
