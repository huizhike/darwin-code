...
`darwin-code-otel` is the OpenTelemetry integration crate for Darwin Code. It provides:

- Provider wiring for log/trace/metric exporters (`darwin_code_otel::OtelProvider`
  and `darwin_code_otel::provider`).
- Session-scoped business event emission via `darwin_code_otel::SessionTelemetry`.
- Low-level metrics APIs via `darwin_code_otel::metrics`.
- Trace-context helpers via `darwin_code_otel::trace_context` and crate-root re-exports.
...
```rust
use darwin_code_otel::config::OtelExporter;
use darwin_code_otel::config::OtelHttpProtocol;
use darwin_code_otel::config::OtelSettings;
use darwin_code_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "darwin-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    darwin_code_home: std::path::PathBuf::from("/tmp"),
...
Darwin Code-specific session events. Rich session/business events should go through
...
`darwin-code-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Darwin Code-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use darwin_code_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("darwin.session_started", 1, &[("source", "tui")])?;
metrics.histogram("darwin.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("darwin.turns", 1, &[("model", "gpt-5.1")])?;
