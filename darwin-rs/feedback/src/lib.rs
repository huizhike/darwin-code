...
#[derive(Clone)]
pub struct DarwinCodeFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for DarwinCodeFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl DarwinCodeFeedback {
    pub fn new() -> Self {
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("darwin-code-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("darwin-code-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = DarwinCodeFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("darwin-code-feedback-extra-{}.jsonl", ThreadId::new());
        let extra_path = std::env::temp_dir().join(&extra_filename);
        fs::write(&extra_path, "rollout").expect("extra attachment should be written");

        let snapshot_with_diagnostics = DarwinCodeFeedback::new()
        ...
        let attachments_without_diagnostics = DarwinCodeFeedback::new()
            .snapshot(/*session_id*/ None)
