...
const RESOURCES_DIRNAME: &str = "darwin-code-resources";
...
pub enum InstallContext {
    Standalone {
        ...
        /// `~/.darwin/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        release_dir: PathBuf,
    ...
    /// A DarwinCode binary launched through the npm-managed `darwin.js` shim.
    Npm,
    /// A DarwinCode binary launched through the bun-managed `darwin.js` shim.
    Bun,
    /// A DarwinCode binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled DarwinCode binaries, custom
    /// internal launchers, and tests that execute DarwinCode from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let darwin_code_home = darwin_code_utils_home_dir::find_darwin_code_home().ok();
        Self::from_exe_with_darwin_code_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
    ...
    fn from_exe_with_darwin_code_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        ...
        if let Some(exe_path) = current_exe
            && let Some(standalone_context) = standalone_install_context(exe_path, darwin_code_home)
        {
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("DARWIN_CODE_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("DARWIN_CODE_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn standalone_install_context(
    ...
    let canonical_darwin_code_home = std::fs::canonicalize(darwin_code_home?).ok()?;
    let release_dir = canonical_exe.parent()?.to_path_buf();
    let releases_root = canonical_darwin_code_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        let darwin_code_home = tempfile::tempdir()?;
        let release_dir = darwin_code_home
            .path()
        ...
        let context = InstallContext::from_exe_with_darwin_code_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*darwin_code_home*/ Some(darwin_code_home.path()),
        );
        assert_eq!(
            context,
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_darwin_code_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/darwin")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
        ...
        let bun_context = InstallContext::from_exe_with_darwin_code_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/darwin")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_darwin_code_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/darwin")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*darwin_code_home*/ None,
        );
