...
- The legacy rule matcher lives in `darwin-code-execpolicy-legacy`.
...
- From the Darwin Code CLI, run `darwin execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
darwin execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
darwin execpolicy check \
...
```bash
cargo run -p darwin-code-execpolicy -- check --rules path/to/policy.rules git status
```
