...
These are entities exit on the darwin backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Darwin Code` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Darwin Code`
   - The core engine of darwin
...
2. `Session`
   - The `Darwin Code`'s current configuration and state
   - `Darwin Code` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Darwin Code` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Darwin Code` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Darwin Code`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Darwin Code`, as `Darwin Code` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Darwin Code` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Darwin Code`)
...
- `Event`
  - These are messages sent on the `EQ` (`Darwin Code` -> UI)
...
The `response_id` returned from each turn matches the provider `response_id` stored in the API's `/responses` endpoint. It can be stored and used in future `Sessions` to resume threads of work.
...
sequenceDiagram
    ...
    box Daemon
    participant darwin as Darwin Code
    ...
    user->>darwin: Op::ConfigureSession
    darwin-code-->>session: create session
    darwin-code->>user: Event::SessionConfigured
