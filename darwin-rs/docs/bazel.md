...
This repository uses Bazel to build the Rust workspace under `darwin-rs`.
...
- `rules_rs` imports third-party crates from `darwin-rs/Cargo.toml` and
  `darwin-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `darwin_code_rust_crate`, which wraps `rust_library`,
...
- Each crate in `darwin-rs/*/BUILD.bazel` typically uses `darwin_code_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `darwin_code_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `darwin_code_rust_crate` to configure it.
