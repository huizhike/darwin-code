...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, DarwinCodeErr::ServerOverloaded));
}
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    let DarwinCodeErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected DarwinCodeErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let DarwinCodeErr::UnexpectedStatus(err) = err else {
        panic!("expected DarwinCodeErr::UnexpectedStatus, got {err:?}");
    };
