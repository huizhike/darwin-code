...
mod tests {
    ...
    fn hook_source_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.darwin/config.toml").abs();
        let dot_darwin_code_folder = test_path_buf("/tmp/worktree/.darwin").abs();
        ...
        assert_eq!(
            super::hook_source_for_config_layer_source(&ConfigLayerSource::Mdm {
                domain: "com.darwin.darwin".to_string(),
