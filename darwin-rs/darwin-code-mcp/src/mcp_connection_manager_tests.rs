...
fn create_darwin_code_apps_tools_cache_context(
    darwin_code_home: PathBuf,
    account_id: Option<&str>,
    provider_user_id: Option<&str>,
) -> DarwinCodeAppsToolsCacheContext {
    DarwinCodeAppsToolsCacheContext {
        darwin_code_home,
...
#[test]
fn declared_darwin_file_fields_treat_names_literally() {
    let meta = serde_json::json!({
        "darwin/fileParams": ["file", "input_file", "attachments"]
    });
    let meta = meta.as_object().expect("meta object");

    assert_eq!(
        declared_darwin_file_input_param_names(Some(meta)),
        vec![
            "file".to_string(),
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(DARWIN_CODE_APPS_MCP_SERVER_NAME, "upload").tool;
...
fn test_qualify_tools_duplicated_names_skipped() {
    let tools = vec![create_test_tool("server1", "duplicate_tool")];
...
#[test]
fn darwin_code_apps_tools_cache_is_overwritten_by_last_write() {
    let darwin_code_home = tempdir().expect("tempdir");
    let cache_context = create_darwin_code_apps_tools_cache_context(
        darwin_code_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(DARWIN_CODE_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(DARWIN_CODE_APPS_MCP_SERVER_NAME, "two")];

    write_cached_darwin_code_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 = read_cached_darwin_code_apps_tools(&cache_context)
        .expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_darwin_code_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 = read_cached_darwin_code_apps_tools(&cache_context)
        .expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn darwin_code_apps_tools_cache_is_scoped_per_user() {
    let darwin_code_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_darwin_code_apps_tools_cache_context(
        darwin_code_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_darwin_code_apps_tools_cache_context(
        darwin_code_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(DARWIN_CODE_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(DARWIN_CODE_APPS_MCP_SERVER_NAME, "two")];

    write_cached_darwin_code_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_darwin_code_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 = read_cached_darwin_code_apps_tools(&cache_context_user_1)
        .expect("cache entry for user one");
    let read_user_2 = read_cached_darwin_code_apps_tools(&cache_context_user_2)
...
#[test]
fn darwin_code_apps_tools_cache_filters_disallowed_connectors() {
    let darwin_code_home = tempdir().expect("tempdir");
    let cache_context = create_darwin_code_apps_tools_cache_context(
        darwin_code_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached =
        read_cached_darwin_code_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn darwin_code_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let darwin_code_home = tempdir().expect("tempdir");
    let cache_context = create_darwin_code_apps_tools_cache_context(
        darwin_code_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": DARWIN_CODE_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(DARWIN_CODE_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_darwin_code_apps_tools(&cache_context).is_none());
}

#[test]
fn darwin_code_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let darwin_code_home = tempdir().expect("tempdir");
    let cache_context = create_darwin_code_apps_tools_cache_context(
        darwin_code_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_darwin_code_apps_tools_loads_from_disk_cache() {
    let darwin_code_home = tempdir().expect("tempdir");
    let cache_context = create_darwin_code_apps_tools_cache_context(
        darwin_code_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_darwin_code_apps_tools(&cache_context, &cached_tools);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    manager.clients.insert(
        DARWIN_CODE_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
    ...
    let tool = tools
        .get("mcp__darwin_code_apps__calendar_create_event")
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, DARWIN_CODE_APPS_MCP_SERVER_NAME);
...
fn elicitation_capability_enabled_for_custom_servers() {
    for server_name in [DARWIN_CODE_APPS_MCP_SERVER_NAME, "custom_mcp"] {
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `darwin mcp login {server_name}`."
    );
