...
use crate::mcp::DARWIN_CODE_APPS_MCP_SERVER_NAME;
...
const DARWIN_CODE_APPS_TOOLS_CACHE_SCHEMA_VERSION: u8 = 2;
const DARWIN_CODE_APPS_TOOLS_CACHE_DIR: &str = "cache/darwin_code_apps_tools";
const MCP_TOOLS_LIST_DURATION_METRIC: &str = "darwin.mcp.tools.list.duration_ms";
const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "darwin.mcp.tools.fetch_uncached.duration_ms";
const MCP_TOOLS_CACHE_WRITE_DURATION_METRIC: &str = "darwin.mcp.tools.cache_write.duration_ms";
...
pub fn darwin_code_apps_tools_cache_key() -> DarwinCodeAppsToolsCacheKey {
    DarwinCodeAppsToolsCacheKey {
        account_id: None,
...
const META_DARWIN_FILE_PARAMS: &str = "darwin/fileParams";

pub fn declared_darwin_file_input_param_names(
    ...
    meta.get(META_DARWIN_FILE_PARAMS)
...
fn tool_with_model_visible_input_schema(tool: &Tool) -> Tool {
    let file_params = declared_darwin_file_input_param_names(tool.meta.as_deref());
...
pub struct DarwinCodeAppsToolsCacheKey {
...
#[derive(Clone)]
struct DarwinCodeAppsToolsCacheContext {
    darwin_code_home: PathBuf,
    user_key: DarwinCodeAppsToolsCacheKey,
}

impl DarwinCodeAppsToolsCacheContext {
    fn cache_path(&self) -> PathBuf {
        ...
        self.darwin_code_home
            .join(DARWIN_CODE_APPS_TOOLS_CACHE_DIR)
            .join(format!("{user_key_hash}.json"))
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct DarwinCodeAppsToolsDiskCache {
    schema_version: u8,
    tools: Vec<ToolInfo>,
}

enum CachedDarwinCodeAppsToolsLoad {
    Hit(Vec<ToolInfo>),
    Missing,
    Invalid,
}
...
struct ManagedClient {
    ...
    darwin_code_apps_tools_cache_context: Option<DarwinCodeAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.darwin_code_apps_tools_cache_context.as_ref()
            && let CachedDarwinCodeAppsToolsLoad::Hit(tools) =
                load_cached_darwin_code_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.darwin_code_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    fn new(
        ...
        let startup_snapshot = load_startup_cached_darwin_code_apps_tools_snapshot(
            &server_name,
            darwin_code_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = async {
                ...
                match start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        darwin_code_apps_tools_cache_context,
                    },
                )
...
/// MCP server capability indicating that DarwinCode should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "darwin/sandbox-state-meta";
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        darwin_code_home: PathBuf,
        darwin_code_apps_tools_cache_key: DarwinCodeAppsToolsCacheKey,
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let darwin_code_apps_tools_cache_context =
                if server_name == DARWIN_CODE_APPS_MCP_SERVER_NAME {
                    Some(DarwinCodeAppsToolsCacheContext {
                        darwin_code_home: darwin_code_home.clone(),
                        user_key: darwin_code_apps_tools_cache_key.clone(),
                    })
                } else {
                    None
                };
    ...
    /// Force-refresh darwin apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_darwin_code_apps_tools_cache(
        &self,
    ) -> Result<HashMap<String, ToolInfo>> {
        let managed_client = self
            .clients
            .get(DARWIN_CODE_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{DARWIN_CODE_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
            .context("failed to get client")?;
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{DARWIN_CODE_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_darwin_code_apps_tools_if_needed(
            DARWIN_CODE_APPS_MCP_SERVER_NAME,
            managed_client.darwin_code_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
            MCP_TOOLS_LIST_DURATION_METRIC,
...
pub fn filter_non_darwin_code_apps_mcp_tools_only(
    mcp_tools: &HashMap<String, ToolInfo>,
) -> HashMap<String, ToolInfo> {
    mcp_tools
        .iter()
        .filter(|(_, tool)| tool.server_name != DARWIN_CODE_APPS_MCP_SERVER_NAME)
        .map(|(name, tool)| (name.clone(), tool.clone()))
        .collect()
}

fn normalize_darwin_code_apps_tool_title(
    server_name: &str,
...
fn normalize_darwin_code_apps_callable_name(
    server_name: &str,
    tool_name: &str,
...
fn normalize_darwin_code_apps_callable_namespace(
    server_name: &str,
    connector_name: Option<&str>,
) -> String {
    if server_name == DARWIN_CODE_APPS_MCP_SERVER_NAME
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "darwin-code-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("DarwinCode".into()),
            description: None,
            icons: None,
            website_url: None,
        },
...
fn write_cached_darwin_code_apps_tools_if_needed(
    server_name: &str,
    cache_context: Option<&DarwinCodeAppsToolsCacheContext>,
    tools: &[ToolInfo],
) {
    if server_name != DARWIN_CODE_APPS_MCP_SERVER_NAME {
        return;
    ...
    if let Some(cache_context) = cache_context {
        ...
        write_cached_darwin_code_apps_tools(cache_context, tools);
        emit_duration(
...
fn load_startup_cached_darwin_code_apps_tools_snapshot(
    server_name: &str,
    cache_context: Option<&DarwinCodeAppsToolsCacheContext>,
) -> Option<Vec<ToolInfo>> {
    if server_name != DARWIN_CODE_APPS_MCP_SERVER_NAME {
    ...
    match load_cached_darwin_code_apps_tools(cache_context) {
        CachedDarwinCodeAppsToolsLoad::Hit(tools) => Some(tools),
        CachedDarwinCodeAppsToolsLoad::Missing | CachedDarwinCodeAppsToolsLoad::Invalid => None,
    }
}

#[cfg(test)]
fn read_cached_darwin_code_apps_tools(
    cache_context: &DarwinCodeAppsToolsCacheContext,
) -> Option<Vec<ToolInfo>> {
    match load_cached_darwin_code_apps_tools(cache_context) {
        CachedDarwinCodeAppsToolsLoad::Hit(tools) => Some(tools),
        CachedDarwinCodeAppsToolsLoad::Missing | CachedDarwinCodeAppsToolsLoad::Invalid => None,
...
fn load_cached_darwin_code_apps_tools(
    cache_context: &DarwinCodeAppsToolsCacheContext,
) -> CachedDarwinCodeAppsToolsLoad {
    let cache_path = cache_context.cache_path();
    let bytes = match std::fs::read(cache_path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return CachedDarwinCodeAppsToolsLoad::Missing;
        }
        Err(_) => return CachedDarwinCodeAppsToolsLoad::Invalid,
    };
    let cache: DarwinCodeAppsToolsDiskCache = match serde_json::from_slice(&bytes) {
        Ok(cache) => cache,
        Err(_) => return CachedDarwinCodeAppsToolsLoad::Invalid,
    };
    if cache.schema_version != DARWIN_CODE_APPS_TOOLS_CACHE_SCHEMA_VERSION {
        return CachedDarwinCodeAppsToolsLoad::Invalid;
    }
    CachedDarwinCodeAppsToolsLoad::Hit(filter_disallowed_darwin_code_apps_tools(cache.tools))
}

fn write_cached_darwin_code_apps_tools(
    cache_context: &DarwinCodeAppsToolsCacheContext,
    tools: &[ToolInfo],
) {
    let cache_path = cache_context.cache_path();
    ...
    let tools = filter_disallowed_darwin_code_apps_tools(tools.to_vec());
    let Ok(bytes) = serde_json::to_vec_pretty(&DarwinCodeAppsToolsDiskCache {
        schema_version: DARWIN_CODE_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        tools,
    }) else {
        return;
    };
...
fn filter_disallowed_darwin_code_apps_tools(tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
    tools
        .into_iter()
        .filter(|tool| {
            tool.connector_id
...
async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .into_iter()
        .map(|tool| {
            let callable_name = normalize_darwin_code_apps_callable_name(
                server_name,
                &tool.tool.name,
                tool.connector_id.as_deref(),
                tool.connector_name.as_deref(),
            );
            let callable_namespace = normalize_darwin_code_apps_callable_namespace(
                server_name,
                tool.connector_name.as_deref(),
            );
            ...
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title = normalize_darwin_code_apps_tool_title(
                    server_name,
        ...
        .collect();
    if server_name == DARWIN_CODE_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_darwin_code_apps_tools(tools));
    }
    Ok(tools)
}
