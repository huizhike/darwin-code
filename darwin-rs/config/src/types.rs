//! Types used to define loaded and effective DarwinCode configuration values.
...
/// Settings that govern if and what will be written to `~/.darwin/history.jsonl`.
...
pub struct AppConfig {
    /// When `false`, DarwinCode does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$DARWIN_CODE_HOME/themes` for custom themes.
    #[serde(default)]
...
/// (primarily the DarwinCode IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user has seen the gpt-5.1-darwin-code-max migration prompt
    #[serde(rename = "hide_gpt-5.1-darwin-code-max_migration_prompt")]
    pub hide_gpt_5_1_darwin_code_max_migration_prompt: Option<bool>,
