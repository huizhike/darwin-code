//! Schema-heavy configuration TOML types used by DarwinCode.
...
/// Base config deserialized from ~/.darwin/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by DarwinCode will likely degrade model performance.
    ...
    /// Defaults to `$DARWIN_CODE_SQLITE_HOME` when set. Otherwise uses `$DARWIN_CODE_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where DarwinCode writes log files, for example `darwin-code-tui.log`.
    /// Defaults to `$DARWIN_CODE_HOME/log`.
    ...
    /// directories for `.darwin` folders. Defaults to [".git"] when unset.
    #[serde(default)]
