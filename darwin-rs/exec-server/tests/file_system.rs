...
use darwin_code_exec_server::LocalFileSystem;
use darwin_code_exec_server::ReadDirectoryEntry;
...
use common::exec_server::TestDarwinCodeHelperPaths;
use common::exec_server::exec_server;
#[cfg(target_os = "linux")]
use common::exec_server::exec_server_with_env;
use common::exec_server::test_darwin_code_helper_paths;

struct FileSystemContext {
    file_system: Arc<dyn ExecutorFileSystem>,
    _helper_paths: Option<TestDarwinCodeHelperPaths>,
...
async fn create_file_system_context(use_remote: bool) -> Result<FileSystemContext> {
    if use_remote {
        ...
        let runtime_paths = ExecServerRuntimePaths::new(
            helper_paths.darwin_code_exe.clone(),
            helper_paths.darwin_code_linux_sandbox_exe.clone(),
        )?;
        Ok(FileSystemContext {
...
async fn file_system_sandboxed_write_allows_explicit_alias_roots(use_remote: bool) -> Result<()> {
    ...
    let tmp = tempfile::Builder::new()
        .prefix("darwin-code-fs-sandbox-alias-")
