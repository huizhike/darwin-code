...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestDarwinCodeHelperPaths,
    child: Child,
...
pub(crate) struct TestDarwinCodeHelperPaths {
    pub(crate) darwin_code_exe: PathBuf,
    pub(crate) darwin_code_linux_sandbox_exe: Option<PathBuf>,
}

pub(crate) fn test_darwin_code_helper_paths() -> anyhow::Result<TestDarwinCodeHelperPaths> {
    let (helper_binary, darwin_code_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestDarwinCodeHelperPaths {
        darwin_code_exe: helper_binary,
...
{
    let helper_paths = test_darwin_code_helper_paths()?;
    let darwin_code_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.darwin_code_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("DARWIN_CODE_HOME", darwin_code_home.path());
    child.envs(env);
