# darwin-code-exec-server

`darwin-code-exec-server` is the library backing `darwin exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`darwin-code-utils-pty`.

It provides:

- a CLI entrypoint: `darwin exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `darwin` binary owns hidden helper dispatch for sandboxed
filesystem operations and `darwin-code-linux-sandbox`.

## Transport

The server speaks the shared `darwin-code-app-server-protocol` message envelope on
the wire.
...
- `arg0`: optional argv0 override forwarded to `darwin-code-utils-pty`.
...
hidden helper process launched from the top-level `darwin` executable and
...
`darwin exec-server` command builds these paths from the `darwin` arg0 dispatch
state.
