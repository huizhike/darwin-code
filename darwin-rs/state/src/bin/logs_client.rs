...
#[derive(Debug, Parser)]
#[command(name = "darwin-code-state-logs")]
#[command(about = "Tail DarwinCode logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to DARWIN_CODE_HOME. Defaults to $DARWIN_CODE_HOME or ~/.darwin.
    #[arg(long, env = "DARWIN_CODE_HOME")]
    darwin_code_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --darwin-code-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let darwin_code_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(darwin_code_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let darwin_code_home = args
        .darwin_code_home
        .clone()
        .unwrap_or_else(default_darwin_code_home);
    Ok(darwin_code_state::logs_db_path(darwin_code_home.as_path()))
}

fn default_darwin_code_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".darwin");
    }
    PathBuf::from(".darwin")
}
