...
impl StateRuntime {
    /// Initialize the state runtime using the provided DarwinCode home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `darwin_code_home`,
    ...
    pub async fn init(
        ...
        .await;
        let state_path = state_db_path(darwin_code_home.as_path());
        let logs_path = logs_db_path(darwin_code_home.as_path());
    ...
    /// Return the configured DarwinCode home directory for this runtime.
    pub fn darwin_code_home(&self) -> &Path {
...
pub fn state_db_path(darwin_code_home: &Path) -> PathBuf {
    darwin_code_home.join(state_db_filename())
}
...
pub fn logs_db_path(darwin_code_home: &Path) -> PathBuf {
    darwin_code_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(darwin_code_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read darwin_code_home for {db_label} db cleanup {}: {err}",
...
mod tests {
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&darwin_code_home)
            .await
            .expect("create darwin home");
        let state_path = state_db_path(darwin_code_home.as_path());
