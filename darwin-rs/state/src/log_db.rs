...
//!
//! # async fn example(state_db: std::sync::Arc<darwin_code_state::StateRuntime>) {
...
mod tests {
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let darwin_code_home =
            std::env::temp_dir().join(format!("darwin-code-state-log-db-{}", Uuid::new_v4()));
