...
mod tests {
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        ...
        let mut metadata =
            test_thread_metadata(&darwin_code_home, thread_id, darwin_code_home.clone());
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&darwin_code_home, thread_id, darwin_code_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                ...
                repository_url: Some("git@example.com:darwin/darwin-code.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:darwin/darwin-code.git")
        );
    }
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:darwin/darwin-code.git")),
            )
            .await
    ...
    async fn update_thread_git_info_can_clear_fields() {
        ...
        metadata.git_origin_url = Some("git@example.com:darwin/darwin-code.git".to_string());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&darwin_code_home, first_id, darwin_code_home.clone());
        first.updated_at = updated_at;
        let mut second =
            test_thread_metadata(&darwin_code_home, second_id, darwin_code_home.clone());
        ...
        let mut older = test_thread_metadata(&darwin_code_home, older_id, darwin_code_home.clone());
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let darwin_code_home = unique_temp_dir();
        let runtime = StateRuntime::init(darwin_code_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
