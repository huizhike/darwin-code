//! Custom CA handling for DarwinCode outbound HTTP and websocket clients.
//!
//! DarwinCode constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `DARWIN_CODE_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because DarwinCode itself commonly runs spawned test processes under seatbelt, so this is not just
...
pub const DARWIN_CODE_CA_CERT_ENV: &str = "DARWIN_CODE_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set DARWIN_CODE_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors DarwinCode custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `DARWIN_CODE_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound DarwinCode HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a DarwinCode custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `DARWIN_CODE_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the DarwinCode custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        darwin_code_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `DARWIN_CODE_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the DarwinCode-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(DARWIN_CODE_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: DARWIN_CODE_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for DarwinCode CA overrides.
    ///
    /// This accepts a few common real-world variants so DarwinCode behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// DarwinCode only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    use super::DARWIN_CODE_CA_CERT_ENV;
    use super::EnvSource;
    ...
    #[test]
    fn ca_path_prefers_darwin_code_env() {
        let env = map_env(&[
            (DARWIN_CODE_CA_CERT_ENV, "/tmp/darwin.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/darwin.pem"))
        );
    }
