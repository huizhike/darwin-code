...
fn write_user_skill(darwin_code_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = darwin_code_home.path().join("skills").join(dir);
...
fn user_config_layer(darwin_code_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(darwin_code_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(darwin_code_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(darwin_code_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(darwin_code_home, user_config_toml),
            ConfigLayerEntry::new(
...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    ...
    let stale_system_skill_dir = darwin_code_home.path().join("skills/.system/stale-skill");
    ...
    let _skills_manager = SkillsManager::new(
        darwin_code_home.path().abs(),
        /*bundled_skills_enabled*/ false,
    );

    assert!(
        !darwin_code_home.path().join("skills/.system").exists(),
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&darwin_code_home, "");
    let skills_manager = SkillsManager::new(
        darwin_code_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

    write_user_skill(&darwin_code_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&darwin_code_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_manager, &cwd, &config_layer_stack, &[]).await;
...
async fn skills_for_cwd_loads_repo_user_and_extra_roots_with_local_fs() {
    ...
    let cwd = tempfile::tempdir().expect("tempdir");
    let extra_root = tempfile::tempdir().expect("tempdir");
    let repo_dot_darwin = cwd.path().join(".darwin");
    fs::create_dir_all(&repo_dot_darwin).expect("create repo config dir");
    ...
    let repo_skill_dir = repo_dot_darwin.join("skills/repo");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&darwin_code_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_darwin_code_folder: repo_dot_darwin.abs(),
                },
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let darwin_code_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = darwin_code_home.path().join("skills/.system/bundled-skill");
    ...
    let config_layer_stack = config_stack(&darwin_code_home, "[skills.bundled]\nenabled = false\n");
    let skills_manager = SkillsManager::new(
        darwin_code_home.path().abs(),
        /*bundled_skills_enabled*/ false,
    );
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    let darwin_code_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let skill_dir = darwin_code_home.path().join("skills").join("demo");
    ...
    let parent_stack = config_stack(&darwin_code_home, &disabled_skill_config);
