...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "darwin-code": ["darwin-code", *DARWIN_CODE_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "darwin-code": [],
    "darwin-code-linux-x64": ["darwin-code", "rg"],
    "darwin-code-linux-arm64": ["darwin-code", "rg"],
    "darwin-code-darwin-x64": ["darwin-code", "rg"],
...
PACKAGE_TARGET_FILTERS: dict[str, str] = {
    ...
    for package_name, package_config in DARWIN_CODE_PLATFORM_PACKAGES.items()
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the Darwin-Code CLI npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="darwin-code",
        help="Which npm package to stage (default: darwin-code).",
    )
    parser.add_argument(
        "--version",
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="darwin-code-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "darwin-code":
        ...
        shutil.copy2(DARWIN_CODE_CLI_ROOT / "bin" / "darwin-code.js", bin_dir / "darwin-code.js")
        ...
        package_json_path = DARWIN_CODE_CLI_ROOT / "package.json"
    elif package in DARWIN_CODE_PLATFORM_PACKAGES:
        platform_package = DARWIN_CODE_PLATFORM_PACKAGES[package]
        ...
        with open(DARWIN_CODE_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
            darwin_code_package_json = json.load(fh)

        package_json = {
            "name": DARWIN_CODE_NPM_NAME,
            "version": platform_version,
            "license": darwin_code_package_json.get("license", "Apache-2.0"),
            "os": [platform_package["os"]],
            "cpu": [platform_package["cpu"]],
            "files": ["vendor"],
            "repository": darwin_code_package_json.get("repository"),
        }

        engines = darwin_code_package_json.get("engines")
    ...
    if package == "darwin-code":
        ...
        package_json["optionalDependencies"] = {
            DARWIN_CODE_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{DARWIN_CODE_NPM_NAME}@"
                f"{compute_platform_package_version(version, DARWIN_CODE_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["darwin-code"]
            if platform_package != "darwin-code"
        }
        ...
        if not isinstance(dependencies, dict):
            dependencies = {}
        dependencies[DARWIN_CODE_NPM_NAME] = version
...
def stage_darwin_code_sdk_sources(staging_dir: Path) -> None:
    package_root = DARWIN_CODE_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("darwin-code-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="darwin-code-npm-pack-") as pack_dir_str:
