...
pub(super) async fn test_config() -> Config {
    ...
    let mut config = ConfigBuilder::default()
        ...
        .await
        .expect("config");
    config.sqlite_home = darwin_code_home.clone();
    config.log_dir = darwin_code_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let (draw_tx, _draw_rx) = tokio::sync::broadcast::channel(16);
    let mut widget = ChatWidget::new_with_op_target(
        ChatWidgetInit {
            config: cfg,
            frame_requester: FrameRequester::new(draw_tx),
            ...
            is_first_run: false,
            status_account_display: Some(crate::status::StatusAccountDisplay::ApiKey),
        ...
        DarwinCodeOpTarget::Direct(op_tx),
    );
    widget.turn_sleep_inhibitor = SleepInhibitor::new(prevent_idle_sleep);
    widget.set_model(&resolved_model);
    while rx.try_recv().is_ok() {
        // Discard constructor-time history/header events so individual tests
        // can assert only the event produced by the action under test.
    }
    (widget, rx, op_rx)
}
...
pub(crate) fn set_byok_model_catalog(chat: &mut ChatWidget) {
    chat.has_account_backed_features = false;
...
fn test_model_info(slug: &str, priority: i32, supports_fast_mode: bool) -> ModelInfo {
    let additional_speed_tiers = if supports_fast_mode {
        vec![darwin_code_protocol::openai_models::SPEED_TIER_FAST]
    } else {
        Vec::new()
    };
...
pub(super) fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> =
        darwin_code_shell_command::parse_command::parse_command(&command);
...
pub(super) fn plugins_test_curated_marketplace(
    ...
    PluginMarketplaceEntry {
        ...
        path: Some(plugins_test_absolute_path("marketplaces/byok")),
        interface: Some(MarketplaceInterface {
            display_name: Some("Darwin Marketplace".to_string()),
        }),
        plugins,
    }
}
...
pub(super) fn plugins_test_detail(
    ...
    PluginDetail {
        marketplace_name: "Darwin Marketplace".to_string(),
        marketplace_path: plugins_test_absolute_path("marketplaces/byok"),
        summary,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    chat.handle_darwin_code_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookStarted(darwin_code_protocol::protocol::HookStartedEvent {
            turn_id: None,
            run: darwin_code_protocol::protocol::HookRunSummary {
                id: run_id.to_string(),
                event_name,
                handler_type: darwin_code_protocol::protocol::HookHandlerType::Command,
                execution_mode: darwin_code_protocol::protocol::HookExecutionMode::Sync,
                scope: darwin_code_protocol::protocol::HookScope::Turn,
                source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
                source: darwin_code_protocol::protocol::HookSource::User,
                display_order: 0,
                status: darwin_code_protocol::protocol::HookRunStatus::Running,
    ...
    chat.handle_darwin_code_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookCompleted(darwin_code_protocol::protocol::HookCompletedEvent {
            turn_id: None,
            run: darwin_code_protocol::protocol::HookRunSummary {
                id: run_id.to_string(),
                event_name,
                ...
                source: darwin_code_protocol::protocol::HookSource::User,
                display_order: 0,
                status: darwin_code_protocol::protocol::HookRunStatus::Completed,
                status_message: Some(status_message.to_string()),
                started_at: 1,
                ...
                entries: vec![
                    darwin_code_protocol::protocol::HookOutputEntry {
                        kind: darwin_code_protocol::protocol::HookOutputEntryKind::Warning,
                        text: "Heads up from the hook".to_string(),
                    },
                    darwin_code_protocol::protocol::HookOutputEntry {
                        kind: darwin_code_protocol::protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: darwin_code_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        darwin_code_protocol::protocol::HookEventName::PreToolUse => "PreToolUse",
        darwin_code_protocol::protocol::HookEventName::PermissionRequest => "PermissionRequest",
        darwin_code_protocol::protocol::HookEventName::PostToolUse => "PostToolUse",
        darwin_code_protocol::protocol::HookEventName::SessionStart => "SessionStart",
        darwin_code_protocol::protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
