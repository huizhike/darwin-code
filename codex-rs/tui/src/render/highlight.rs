...
//! | `DARWIN_CODE_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static DARWIN_CODE_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), darwin_code_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let darwin_code_home_set_ok = DARWIN_CODE_HOME.set(darwin_code_home.clone()).is_ok();
    ...
    if !override_set_ok || !darwin_code_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, darwin_code_home: Option<&Path>) -> Option<String> {
    ...
    let custom_theme_path_display = darwin_code_home
        ...
        .unwrap_or_else(|| format!("$DARWIN_CODE_HOME/themes/{name}.tmTheme"));
    ...
    if let Some(home) = darwin_code_home {
...
fn load_custom_theme(name: &str, darwin_code_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, darwin_code_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, darwin_code_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {DARWIN_CODE_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = darwin_code_home
...
fn build_default_theme() -> Theme {
    ...
    let darwin_code_home = DARWIN_CODE_HOME
        .get()
        .and_then(|darwin_code_home| darwin_code_home.as_deref());
    resolve_theme_with_override(name, darwin_code_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = DARWIN_CODE_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, darwin_code_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{DARWIN_CODE_HOME}/themes/`.
...
/// found in `{darwin_code_home}/themes/`.
pub(crate) fn list_available_themes(darwin_code_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*darwin_code_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*darwin_code_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*darwin_code_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*darwin_code_home*/ None).is_none());
    }
