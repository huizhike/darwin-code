...
fn extract_log_field_handles_empty_bare_values() {
    let line = "event.name=\"darwin_code.tool_result\" mcp_server= mcp_server_origin=";
...
fn extract_log_field_does_not_confuse_similar_keys() {
    let line = "event.name=\"darwin_code.tool_result\" mcp_server_origin=stdio";
...
async fn responses_api_emits_api_request_event() {
    ...
    let TestDarwinCode { darwin_code, .. } = test_darwin_code().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("darwin_code.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected darwin_code.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("darwin_code.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected darwin_code.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("darwin_code.sse_event")
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing darwin_code.sse_event".to_string()))
    });
}
    ...
    let TestDarwinCode { darwin_code, .. } = test_darwin_code()
        .with_config(move |config| {
            config
                ...
                .disable(Feature::GhostCommit)
                .expect("test config should allow feature update");
            // This test only asserts OTEL fields for an unsupported function
            // call. Shell snapshot startup is unrelated and can exhaust process
            // resources under the full workspace test fan-out.
            config
                .features
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    wait_for_event(&darwin_code, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("darwin_code.tool_result")
                    && line.contains("call_id=custom-tool-call")
            })
            .ok_or_else(|| "missing darwin_code.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("darwin_code.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing darwin_code.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_local_shell_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("darwin_code.tool_result") && line.contains("call_id=shell-call")
            })
            .ok_or_else(|| "missing darwin_code.tool_result event".to_string())?;
...
fn tool_decision_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("darwin_code.tool_decision")
                    && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing darwin_code.tool_decision event for {call_id}"))?;
...
async fn handle_container_exec_user_approved_records_tool_decision() {
    ...
    let approval_event = wait_for_event(&darwin_code, |ev| {
