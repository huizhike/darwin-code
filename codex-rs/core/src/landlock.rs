...
use darwin_code_sandboxing::landlock::allow_network_for_proxy;
...
/// (darwin-code-linux-sandbox), which defaults to bubblewrap for filesystem
...
pub async fn spawn_command_under_linux_sandbox<P>(
    darwin_code_linux_sandbox_exe: P,
    command: Vec<String>,
    ...
    P: AsRef<Path>,
{
    ...
    let darwin_code_linux_sandbox_exe = darwin_code_linux_sandbox_exe.as_ref();
    ...
    let arg0 = if darwin_code_linux_sandbox_exe
        .file_name()
        ...
        == Some(DARWIN_CODE_LINUX_SANDBOX_ARG0)
    {
        ...
        darwin_code_linux_sandbox_exe.to_string_lossy().into_owned()
    } else {
        DARWIN_CODE_LINUX_SANDBOX_ARG0.to_string()
    };
    spawn_child_async(SpawnChildRequest {
        program: darwin_code_linux_sandbox_exe.to_path_buf(),
        args,
        arg0: Some(&arg0),
        cwd: command_cwd,
        network_sandbox_policy,
