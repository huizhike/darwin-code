...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let darwin_code_home = tempdir()?;
    std::fs::write(
        darwin_code_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "\n  global instructions  \n",
    )?;
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    ...
    let global_agents_override_path = darwin_code_home.path().join(LOCAL_AGENTS_MD_FILENAME);
...
async fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_root = darwin_code_home.path().join("memories").abs();
    assert_eq!(
...
#[tokio::test]
async fn sqlite_home_defaults_to_darwin_code_home_for_workspace_write() -> std::io::Result<()> {
    let darwin_code_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, darwin_code_home.path().to_path_buf());
...
async fn workspace_write_always_includes_memories_root_once() -> std::io::Result<()> {
    let darwin_code_home = TempDir::new()?;
    let memories_root = darwin_code_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let darwin_code_home = TempDir::new()?;
    let managed_path = darwin_code_home.path().join("managed_config.toml");
    ...
    let cwd = darwin_code_home.path().abs();
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(darwin_code_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(darwin_code_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(darwin_code_home.path())
        .await
...
async fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            darwin_code_home: fixture.darwin_code_home(),
            sqlite_home: fixture.darwin_code_home().to_path_buf(),
            log_dir: fixture.darwin_code_home().join("log").to_path_buf(),
