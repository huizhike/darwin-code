...
#[cfg(target_os = "macos")]
use darwin_code_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use darwin_code_sandboxing::seatbelt::create_seatbelt_command_args;
...
async fn run_command_under_sandbox(
    ...
    if let SandboxType::Windows = sandbox_type {
        #[cfg(target_os = "windows")]
        {
            ...
            use darwin_code_windows_sandbox::run_windows_sandbox_capture;
            ...
            let base_dir = config.darwin_code_home.clone();
                ...
                if use_elevated {
                    run_windows_sandbox_capture_elevated(
                        darwin_code_windows_sandbox::ElevatedSandboxCaptureRequest {
                            ...
                            darwin_code_home: base_dir.as_path(),
                            command: command_vec,
                            cwd: &cwd_clone,
                            env_map,
                            timeout_ms: None,
...
async fn build_debug_sandbox_config(
    ...
    if let Some(darwin_code_home) = darwin_code_home {
        builder = builder
            .darwin_code_home(darwin_code_home.clone())
            .fallback_cwd(Some(darwin_code_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    fn write_permissions_profile_config(
        ...
        std::fs::write(darwin_code_home.path().join("config.toml"), config)?;
        Ok(())
    }
