...
use darwin_code_app_server_protocol::ConfigEdit;
...
use darwin_code_app_server_protocol::ToolsV2;
...
use darwin_code_protocol::config_types::WebSearchContextSize;
use darwin_code_protocol::config_types::WebSearchLocation;
...
fn write_config(darwin_code_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let darwin_code_home_path = darwin_code_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(darwin_code_home_path.join("config.toml"))?;
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let darwin_code_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let darwin_code_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(darwin_code_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[darwin_code_app_server_protocol::ConfigLayer],
