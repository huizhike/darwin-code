...
use darwin_code_protocol::account::PlanType as AccountPlanType;
...
const LOGIN_ISSUER_ENV_VAR: &str = "DARWIN_CODE_APP_SERVER_LOGIN_ISSUER";
...
fn create_config_toml(darwin_code_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(darwin_code_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(darwin_code_home.path().join("auth.json").exists());
    ...
    assert!(
        !darwin_code_home.path().join("auth.json").exists(),
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(darwin_code_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<darwin_code_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(darwin_code_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![darwin_code_app_server_protocol::UserInput::Text {
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/darwin-code/device"));
